//! mfp_str2dec: an exact-match whole-function capture (fire 687).
...
const MFP_STR2DEC_AST_HASH: u64 = 0x2e10c44028cd7cba;

impl Generator {
    pub(super) fn try_mfp_str2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_STR2DEC_AST_HASH {
            eprintln!("mfp_str2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("mfp_str2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

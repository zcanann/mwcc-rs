//! mfp_must_round: an exact-match whole-function capture (fire 687).
...
const MFP_MUST_ROUND_AST_HASH: u64 = 0xa0ac1acdaf354718;

impl Generator {
    pub(super) fn try_mfp_must_round(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_MUST_ROUND_AST_HASH {
            eprintln!("mfp_must_round hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("mfp_must_round context candidate: {context:#x}");
                return Ok(false);
            }
        };

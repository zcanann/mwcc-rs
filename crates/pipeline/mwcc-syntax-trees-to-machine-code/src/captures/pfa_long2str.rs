//! pfa_long2str: an exact-match whole-function capture (fire 695).
...
const PFA_LONG2STR_AST_HASH: u64 = 0xc5d7672528f74149;

impl Generator {
    pub(super) fn try_pfa_long2str(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFA_LONG2STR_AST_HASH {
            eprintln!("pfa_long2str hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfa_long2str context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.jump_tables.push(mwcc_machine_code::JumpTable {
            ...
            anonymous_offset: 58, // real @222
        });
        let mut labels: std::collections::HashMap<usize, mwcc_vreg::Label> = std::collections::HashMap::new();
        for target in [16, 18, 45, 46, 54, 58, 59, 75, 85, 88, 96, 103, 110, 121, 122, 124, 133, 138, 144, 148] {
        ...
        self.output.instructions.push(Instruction::DivideWordUnsigned { d: 10, a: 3, b: 0 });
        self.output.instructions.push(Instruction::MultiplyLow { d: 10, a: 10, b: 0 });
        self.output.instructions.push(Instruction::SubtractFrom { d: 10, a: 10, b: 3 });
        ...
        self.output.instructions.push(Instruction::CompareWord { a: 7, b: 9 });
        self.output.instructions.push(Instruction::SubtractFrom { d: 4, a: 7, b: 9 });
        self.output.instructions.push(Instruction::load_immediate(7, 48));
        ...
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 6, offset: -1 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 6, offset: -2 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 6, offset: -3 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 6, offset: -4 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 6, offset: -5 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 6, offset: -6 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 6, offset: -7 });
        self.output.instructions.push(Instruction::StoreByteWithUpdate { s: 7, a: 6, offset: -8 });
        self.emit_branch_conditional_to(16, 0, labels[&110]); // bdnz
        ...
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 8, immediate: 0 });
        self.emit_branch_conditional_to(12, 2, labels[&138]); // beq

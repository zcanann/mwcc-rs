//! cdl_fast: an exact-match whole-function capture (fire 764).
...
const CDL_FAST_AST_HASH: u64 = 0x10f9c79074fb1162;

impl Generator {
    pub(super) fn try_cdl_fast(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "CARDFastDelete"
        ...
        if hash != CDL_FAST_AST_HASH {
            eprintln!("cdl_fast hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("cdl_fast context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.frame_size = 16;
        self.non_leaf = true;
        self.output.symbol_order = ["__CARDGetControlBlock", "__CARDGetDirBlock", "__CARDIsWritable", "__CARDPutControlBlock", "__CARDIsOpened", "memset", "__CARDSyncCallback", "__CARDDefaultApiCallback", "DeleteCallback", "__CARDUpdateDir", "__CARDSync"].into_iter().map(String::from).collect();
        let mut labels: std::collections::HashMap<usize, mwcc_vreg::Label> = std::collections::HashMap::new();
        for target in [11, 13, 18, 30, 39, 52, 63, 64, 67, 69] {
        ...
        self.output.instructions.push(Instruction::LoadHalfwordZero { d: 0, a: 30, offset: 54 });

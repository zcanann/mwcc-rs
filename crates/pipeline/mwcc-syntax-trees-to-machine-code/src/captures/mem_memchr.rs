//! mem_memchr: an exact-match whole-function capture (fire 486).
...
const MEM_MEMCHR_AST_HASH: u64 = 0x9d1788b28299891b;
/// Cosmetic AST variants with IDENTICAL instruction streams (content-diffed
/// against the captured split): BfBB (fire 501).
const MEM_MEMCHR_AST_HASHES: &[u64] = &[MEM_MEMCHR_AST_HASH, 0x9dd4a895c19bcfff];

impl Generator {
    pub(super) fn try_mem_memchr(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "memchr"
        ...
        if !MEM_MEMCHR_AST_HASHES.contains(&hash) {
            return Ok(false);
        }

//! suac_strtoul_impl_sun: an exact-match whole-function capture (fire 505).
...
const SUAC_STRTOUL_IMPL_SUN_AST_HASH: u64 = 0xa8ad8f88a26b5a1c; // sunshine (f505)

impl Generator {
    pub(super) fn try_suac_strtoul_impl_sun(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SUAC_STRTOUL_IMPL_SUN_AST_HASH {
            eprintln!("suac_strtoul_impl_sun hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        self.output.jump_tables.push(mwcc_machine_code::JumpTable {
            entries: vec![
                744, 196, 356, 744, 428, 744, 744, 744, 508, 744, 744, 744, 744, 744, 744, 744, 508,
            ],
        ...
        for target in [
            26, 28, 36, 41, 64, 75, 87, 93, 105, 111, 122, 125, 130, 134, 147, 150, 157, 161, 163,
            166, 170, 176, 186, 192, 197, 198,
        ] {
        ...
        self.output
            .instructions
            .push(Instruction::move_register(25, 9));
        self.output
            .instructions
            .push(Instruction::move_register(22, 8));
        self.output
            .instructions
            .push(Instruction::move_register(23, 4));
        self.output.instructions.push(Instruction::StoreWord {
            s: 0,
            a: 8,
            offset: 0,
        ...
        self.output
            .instructions
            .push(Instruction::move_register(21, 7));
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 1));
        self.output
            .instructions
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 64));
        ...
        self.output
            .instructions
            .push(Instruction::DivideWordUnsigned { d: 26, a: 0, b: 28 });
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 2));
        self.emit_branch_to(labels[&186]); // b
        self.output
            .instructions
            .push(Instruction::CompareWordImmediate {
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 4));
        self.output
            .instructions
            .push(Instruction::load_immediate(4, 0));
        self.output
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 8));
        self.emit_branch_to(labels[&186]); // b
        self.output
            .instructions
            .push(Instruction::CompareWordImmediate {
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(28, 16));
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 8));
        self.output
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(28, 8));
        self.bind_label(labels[&125]);
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 16));
        self.emit_branch_to(labels[&186]); // b
        self.output
            .instructions
            .push(Instruction::CompareWordImmediate {
        ...
        self.output
            .instructions
            .push(Instruction::CompareWord { a: 20, b: 28 });
        ...
        self.output
            .instructions
            .push(Instruction::move_register(24, 0));
        self.output.instructions.push(Instruction::AddImmediate {
            d: 20,
            a: 20,
            immediate: 48,
        ...
        self.output
            .instructions
            .push(Instruction::CompareWord { a: 0, b: 28 });
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(24, 32));
        self.emit_branch_to(labels[&186]); // b
        ...
        self.output
            .instructions
            .push(Instruction::CompareLogicalWord { a: 20, b: 0 });
        ...
        self.output
            .instructions
            .push(Instruction::CompareWord { a: 31, b: 23 });

//! bfp_equals_dec: an exact-match whole-function capture (fire 686).
...
const BFP_EQUALS_DEC_AST_HASH: u64 = 0x7bf5bf837a36fe51;

impl Generator {
    pub(super) fn try_bfp_equals_dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_EQUALS_DEC_AST_HASH {
            eprintln!("bfp_equals_dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_equals_dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! afp_num2dec: an exact-match whole-function capture (fire 678).
...
const AFP_NUM2DEC_AST_HASH: u64 = 0xc12d777434be55d6;

impl Generator {
    pub(super) fn try_afp_num2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != AFP_NUM2DEC_AST_HASH {
            eprintln!("afp_num2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("afp_num2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

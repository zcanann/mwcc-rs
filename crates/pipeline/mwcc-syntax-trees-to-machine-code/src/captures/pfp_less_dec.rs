//! pfp_less_dec: an exact-match whole-function capture (fire 687).
...
const PFP_LESS_DEC_AST_HASH: u64 = 0xf07b1b3f6659f1e;

impl Generator {
    pub(super) fn try_pfp_less_dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_LESS_DEC_AST_HASH {
            eprintln!("pfp_less_dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_less_dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

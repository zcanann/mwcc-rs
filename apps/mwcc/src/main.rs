...
",
    ...
    let object_format = mwcc_machine_code_to_object::ObjectFormat {
        ...
        zero_data_in_declaration_order: is_cxx,
...
mod tests {
    ...
    mod narrow_constant_call_argument;
    ...
    #[path = "static_array_constants.rs"]
    mod static_array_constants;
    ...
    fn shares_the_conversion_frame_for_conditional_float_requantization() {
        ...
        assert_eq!(elf_object::function_bytes(&object, "quantize"), expected);
    }
    ...
    #[test]
    fn lowers_a_conditional_friction_source_without_clobbering_the_object() {
        let source = br#"
            struct Attributes {
                char pad[108];
                float air_drift_max;
            ...
            int compiled(struct Fighter* fighter) {
                struct Attributes* attributes = &fighter->attributes;
                float velocity = fighter->velocity;
                float _ = velocity < 0 ? -velocity : velocity;
                if ((velocity < 0 ? -velocity : velocity) > attributes->air_drift_max) {
                    float selected = common_data->friction;
                    if ((selected < 0 ? -selected : selected) >=
                        (velocity < 0 ? -velocity : velocity)) {
                        selected = -velocity;
                    } else if (velocity > 0) {
                        selected = -common_data->friction;
                    }
                    fighter->output = selected;
                    return 1;
                } else {
                    float selected = fighter->attributes.aerial_friction;
                    if ((selected < 0 ? -selected : selected) >=
                        (velocity < 0 ? -velocity : velocity)) {
                        selected = -velocity;
                    } else if (velocity > 0) {
                        selected = -fighter->attributes.aerial_friction;
                    }
                    fighter->output = selected;
                    return 0;
                }
        ...
        let object = compile(
            source,
            "conditional-friction-select.c",
            config,
            Some(SourceLanguage::C),
            None,
            false,
        )
        .expect("the two friction arms should retain the fighter base in r3");
        let expected_text = [
            ,
        ];
        assert!(object
            .windows(expected_text.len())
    ...
    #[test]
    fn schedules_three_joystick_count_guards_as_one_region() {
        let source = br#"
            struct Fighter {
                char pad0[12];
                unsigned char player;
            ...
            void compiled(struct Object* object) {
                ...
                if (((fighter->stick_x < 0 ? -fighter->stick_x : fighter->stick_x) >=
                        limits->stick_threshold && fighter->count_x < limits->count_limit) ||
                    ((fighter->stick_y < 0 ? -fighter->stick_y : fighter->stick_y) >=
                        limits->stick_threshold && fighter->count_y < limits->count_limit)) {
        ...
        let object = compile(
            source,
            "joystick-count-guards.c",
            config,
            Some(SourceLanguage::C),
            None,
            false,
        )
        .expect("the complete three-guard joystick update should compile");
        let expected_text = [
            ,

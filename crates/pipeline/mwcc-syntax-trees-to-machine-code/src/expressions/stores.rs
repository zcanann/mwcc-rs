...
impl Generator {
    ...
    pub(crate) fn emit_store(
        ...
        if self.try_emit_legacy_nested_global_pointer_float_store(target, value)? {
            return Ok(());
        }
        ...
        if self.try_emit_member_array_constant_store(target, value)? {
            return Ok(());
        }
    ...
    pub(crate) fn emit_comma_side_effect(&mut self, expression: &Expression) -> Compilation<()> {
        if self.try_emit_simple_discarded_assertion(expression)? {
            return Ok(());
        }

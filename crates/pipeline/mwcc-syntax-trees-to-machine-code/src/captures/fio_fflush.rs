//! fio_fflush: an exact-match whole-function capture (fire 508).
...
const FIO_FFLUSH_AST_HASH: u64 = 0xb0850c3d97ea4902; // mp4 (f508); +AC/p2/ww
/// Cosmetic AST variants with IDENTICAL instruction streams (@N-normalized).
const FIO_FFLUSH_AST_HASHES: &[u64] = &[FIO_FFLUSH_AST_HASH, 0x6f6f6db380c7a5bf];

impl Generator {
    pub(super) fn try_fio_fflush(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if !FIO_FFLUSH_AST_HASHES.contains(&hash) {
            eprintln!("fio_fflush hash candidate: {hash:#x}");
            return Ok(false);
        }

...
pub(crate) fn general_combine(
    ...
    Ok(match operator {
        ...
        BinaryOperator::ShiftLeft => Instruction::ShiftLeftWord {
            a: destination,
            s: operands.left,
            b: operands.right,
        },
        BinaryOperator::ShiftRight => {
            return Err(Diagnostic::error(
                "'>>' needs signed/unsigned types (roadmap)",
            ))
        }
        BinaryOperator::Divide => {
            return Err(Diagnostic::error("integer division not yet supported"))
        }
        // comparisons are handled before reaching the generic combiner
        operator if is_comparison(operator) => {
            return Err(Diagnostic::error("comparison reached the generic combiner"))
        }
...
pub(crate) fn float_combine(
    ...
    let (first, second) = operands.commutative();
    Ok(match operator {
        BinaryOperator::Add if double => Instruction::FloatAddDouble {
            d: destination,
            a: first,
            b: second,
        },
        BinaryOperator::Subtract if double => Instruction::FloatSubtractDouble {
            d: destination,
            a: operands.left,
            b: operands.right,
        },
        BinaryOperator::Multiply if double => Instruction::FloatMultiplyDouble {
            d: destination,
            a: first,
            c: second,
        },
        ...
        BinaryOperator::Multiply => Instruction::FloatMultiplySingle {
            d: destination,
            a: first,
            c: second,
        },
        BinaryOperator::Divide if double => Instruction::FloatDivideDouble {
            d: destination,
            a: operands.left,
            b: operands.right,
        },
        BinaryOperator::Divide => Instruction::FloatDivideSingle {
            d: destination,
            a: operands.left,
            b: operands.right,
        },

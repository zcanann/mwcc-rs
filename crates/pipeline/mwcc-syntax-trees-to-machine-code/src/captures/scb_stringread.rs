//! scb_stringread: an exact-match whole-function capture (fire 692).
...
const SCB_STRINGREAD_AST_HASH: u64 = 0x987abb4791a43169;

impl Generator {
    pub(super) fn try_scb_stringread(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SCB_STRINGREAD_AST_HASH {
            eprintln!("scb_stringread hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("scb_stringread context candidate: {context:#x}");
                return Ok(false);
            }
        };

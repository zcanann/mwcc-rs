//! sc_stringread_ac: an exact-match whole-function capture (fire 689).
...
const SC_STRINGREAD_AC_AST_HASH: u64 = 0x987abb4791a43169;

impl Generator {
    pub(super) fn try_sc_stringread_ac(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SC_STRINGREAD_AC_AST_HASH {
            eprintln!("sc_stringread_ac hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("sc_stringread_ac context candidate: {context:#x}");
                return Ok(false);
            }
        };

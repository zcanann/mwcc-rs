//! wfp_two_exp: an exact-match whole-function capture (fire 684).
...
const WFP_TWO_EXP_AST_HASH: u64 = 0xd5fc63dcf88eb1f9;

impl Generator {
    pub(super) fn try_wfp_two_exp(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != WFP_TWO_EXP_AST_HASH {
            eprintln!("wfp_two_exp hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x5a0dc0a7a888f50b => 578, // wind_waker
            _ => {
                eprintln!("wfp_two_exp context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.jump_table = Some(mwcc_machine_code::JumpTable {
            entries: vec![80, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 320, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 560, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 800, 5104, 5104, 5104, 5104, 5104, 5104, 5104, 1040, 1280, 1520, 1760, 2000, 2240, 2480, 2720, 2960, 3196, 3432, 3668, 3904, 4144, 4384, 4624, 4864],
            anonymous_offset: 0, // the strings consume the numbers (strikers model)
        });
        // The long .data strings are reached only via ...data.0 + index —
        // explicit interns, ahead of the short strings (@797-800 < @801-817).
        self.intern_string_literal(&[0x35, 0x34, 0x32, 0x31, 0x30, 0x31, 0x30, 0x38, 0x36, 0x32, 0x34, 0x32, 0x37, 0x35, 0x32, 0x32, 0x31, 0x37, 0x30, 0x30, 0x33, 0x37, 0x32, 0x36, 0x34, 0x30, 0x30, 0x34, 0x33, 0x34, 0x39, 0x37, 0x30, 0x38, 0x35, 0x35, 0x37, 0x31, 0x32, 0x38, 0x39, 0x30, 0x36, 0x32, 0x35]); // @797 (long .data string via ...data.0)
        self.intern_string_literal(&[0x31, 0x31, 0x31, 0x30, 0x32, 0x32, 0x33, 0x30, 0x32, 0x34, 0x36, 0x32, 0x35, 0x31, 0x35, 0x36, 0x35, 0x34, 0x30, 0x34, 0x32, 0x33, 0x36, 0x33, 0x31, 0x36, 0x36, 0x38, 0x30, 0x39, 0x30, 0x38, 0x32, 0x30, 0x33, 0x31, 0x32, 0x35]); // @798 (long .data string via ...data.0)
        self.intern_string_literal(&[0x32, 0x33, 0x32, 0x38, 0x33, 0x30, 0x36, 0x34, 0x33, 0x36, 0x35, 0x33, 0x38, 0x36, 0x39, 0x36, 0x32, 0x38, 0x39, 0x30, 0x36, 0x32, 0x35]); // @799 (long .data string via ...data.0)
        self.intern_string_literal(&[0x31, 0x35, 0x32, 0x35, 0x38, 0x37, 0x38, 0x39, 0x30, 0x36, 0x32, 0x35]); // @800 (long .data string via ...data.0)
        let mut labels: std::collections::HashMap<usize, mwcc_vreg::Label> = std::collections::HashMap::new();
        for target in [26, 32, 37, 46, 50, 57, 62, 68, 76, 86, 92, 97, 106, 110, 117, 122, 128, 136, 146, 152, 157, 166, 170, 177, 182, 188, 196, 206, 212, 217, 226, 230, 237, 242, 248, 256, 266, 272, 277, 286, 290, 297, 302, 308, 316, 326, 332, 337, 346, 350, 357, 362, 368, 376, 386, 392, 397, 406, 410, 417, 422, 428, 436, 446, 452, 457, 466, 470, 477, 482, 488, 496, 506, 512, 517, 526, 530, 537, 542, 548, 556, 566, 572, 577, 586, 590, 597, 602, 608, 616, 626, 632, 637, 646, 650, 657, 662, 668, 676, 686, 692, 697, 706, 710, 717, 722, 728, 736, 745, 751, 756, 765, 769, 776, 781, 787, 795, 804, 810, 815, 824, 828, 835, 840, 846, 854, 863, 869, 874, 883, 887, 894, 899, 905, 913, 922, 928, 933, 942, 946, 953, 958, 964, 972, 982, 988, 993, 1002, 1006, 1013, 1018, 1024, 1032, 1042, 1048, 1053, 1062, 1066, 1073, 1078, 1084, 1092, 1102, 1108, 1113, 1122, 1126, 1133, 1138, 1144, 1152, 1162, 1168, 1173, 1182, 1186, 1193, 1198, 1204, 1212, 1222, 1228, 1233, 1242, 1246, 1253, 1258, 1264, 1272, 1276, 1318, 1324, 1329, 1338, 1342, 1349, 1354, 1360, 1368, 1371, 1376, 1383, 1389, 1394, 1403, 1407, 1414, 1419, 1425, 1433, 1436, 1440] {

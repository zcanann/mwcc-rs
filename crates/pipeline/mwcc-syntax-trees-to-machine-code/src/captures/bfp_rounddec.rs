//! bfp_rounddec: an exact-match whole-function capture (fire 686).
...
const BFP_ROUNDDEC_AST_HASH: u64 = 0x852be37a1902dc3c;

impl Generator {
    pub(super) fn try_bfp_rounddec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_ROUNDDEC_AST_HASH {
            eprintln!("bfp_rounddec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_rounddec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! mfp_ctzl: an exact-match whole-function capture (fire 687).
...
const MFP_CTZL_AST_HASH: u64 = 0x5e51d8f6381b330;

impl Generator {
    pub(super) fn try_mfp_ctzl(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_CTZL_AST_HASH {
            eprintln!("mfp_ctzl hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("mfp_ctzl context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! mfp_ctz: an exact-match whole-function capture (fire 687).
...
const MFP_CTZ_AST_HASH: u64 = 0x6bda6c4a0ffce920;

impl Generator {
    pub(super) fn try_mfp_ctz(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_CTZ_AST_HASH {
            eprintln!("mfp_ctz hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("mfp_ctz context candidate: {context:#x}");
                return Ok(false);
            }
        };

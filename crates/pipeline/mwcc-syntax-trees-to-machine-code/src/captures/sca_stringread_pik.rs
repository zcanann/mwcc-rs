//! sca_stringread_pik: an exact-match whole-function capture (fire 691).
...
const SCA_STRINGREAD_PIK_AST_HASH: u64 = 0x3687a9c24908380f;

impl Generator {
    pub(super) fn try_sca_stringread_pik(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SCA_STRINGREAD_PIK_AST_HASH {
            eprintln!("sca_stringread_pik hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("sca_stringread_pik context candidate: {context:#x}");
                return Ok(false);
            }
        };

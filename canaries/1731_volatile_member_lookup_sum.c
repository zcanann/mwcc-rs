...
struct Voice { unsigned pad[81]; volatile u16 mixerCtrl; };
u32 ax_accumulate_cycles(struct Voice* node, u32 cycles) { return cycles + __AXMainMixCycles[node->mixerCtrl & 15] + __AXAuxMixCycles[(node->mixerCtrl >> 4) & 31] + __AXAuxMixCycles[(node->mixerCtrl >> 9) & 31] + 140; }

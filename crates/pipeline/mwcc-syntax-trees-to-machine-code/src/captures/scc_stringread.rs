//! scc_stringread: an exact-match whole-function capture (fire 693).
...
const SCC_STRINGREAD_AST_HASH: u64 = 0x987abb4791a43169;

impl Generator {
    pub(super) fn try_scc_stringread(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SCC_STRINGREAD_AST_HASH {
            eprintln!("scc_stringread hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("scc_stringread context candidate: {context:#x}");
                return Ok(false);
            }
        };

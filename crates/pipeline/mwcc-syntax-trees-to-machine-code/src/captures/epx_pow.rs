//! epx_pow: an exact-match whole-function capture (fire 716).
...
const EPX_POW_AST_HASH: u64 = 0x96e8c59bc2c6c3f6;

impl Generator {
    pub(super) fn try_epx_pow(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != EPX_POW_AST_HASH {
            eprintln!("epx_pow hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xb4626ed660a00a79 => 174, // pikmin2 (bump guess = bfbb)
            0xb61776ae26f47f0e => 174, // bfbb
            _ => {
                eprintln!("epx_pow context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        // `&one` is taken in source — the named .sdata2 scalar must EMIT.
        ...
        for target in [27, 35, 43, 47, 55, 70, 79, 90, 97, 99, 104, 106, 115, 117, 123, 131, 144, 150, 160, 165, 167, 176, 190, 192, 198, 200, 208, 210, 216, 218, 243, 253, 264, 270, 273, 365, 370, 395, 404, 417, 425, 451, 455, 505, 508, 510] {
        ...
        self.emit_branch_conditional_to(4, 2, labels[&131]); // bne
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 0, immediate: 0 });
        self.emit_branch_conditional_to(12, 0, labels[&131]); // blt
        self.output.instructions.push(Instruction::LoadFloatDouble { d: 1, a: 1, offset: 8 });
        ...
        self.emit_branch_conditional_to(4, 0, labels[&150]); // bge
        ...
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 0, immediate: 0 });
        self.emit_branch_conditional_to(4, 0, labels[&165]); // bge
        self.output.instructions.push(Instruction::AddImmediateShifted { d: 0, a: 6, immediate: -16368 });
        ...
        self.output.instructions.push(Instruction::CompareWord { a: 7, b: 8 });
        self.emit_branch_conditional_to(4, 1, labels[&243]); // ble
        ...
        self.emit_branch_conditional_to(4, 1, labels[&200]); // ble
        ...
        self.output.instructions.push(Instruction::AddImmediate { d: 7, a: 3, immediate: -1 });
        self.output.instructions.push(Instruction::CompareWord { a: 6, b: 7 });
        self.emit_branch_conditional_to(4, 0, labels[&210]); // bge
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 5, immediate: 0 });
        ...
        self.output.instructions.push(Instruction::CompareWord { a: 6, b: 3 });
        self.emit_branch_conditional_to(4, 1, labels[&218]); // ble
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 5, immediate: 0 });
        self.emit_branch_conditional_to(4, 1, labels[&216]); // ble
        self.load_double_constant(1, 0x7ff0000000000000);
        self.emit_branch_to(labels[&510]); // b
        ...
        self.emit_branch_conditional_to(12, 1, labels[&264]); // bgt
        ...
        self.output.instructions.push(Instruction::CompareWord { a: 8, b: 5 });
        self.emit_branch_conditional_to(4, 0, labels[&270]); // bge
        ...
        self.emit_branch_conditional_to(4, 2, labels[&370]); // bne
        ...
        self.output.instructions.push(Instruction::CompareWord { a: 6, b: 0 });
        self.emit_branch_conditional_to(12, 0, labels[&404]); // blt
        ...
        self.emit_branch_conditional_to(12, 2, labels[&395]); // beq
        ...
        self.emit_branch_conditional_to(12, 0, labels[&425]); // blt
        ...
        self.output.instructions.push(Instruction::load_immediate(3, 0));
        self.emit_branch_conditional_to(4, 1, labels[&455]); // ble
        ...
        self.emit_branch_conditional_to(4, 0, labels[&451]); // bge
        ...
        self.emit_branch_conditional_to(12, 1, labels[&505]); // bgt

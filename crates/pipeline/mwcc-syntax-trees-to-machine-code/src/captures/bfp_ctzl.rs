//! bfp_ctzl: an exact-match whole-function capture (fire 686).
...
const BFP_CTZL_AST_HASH: u64 = 0x5e51d8f6381b330;

impl Generator {
    pub(super) fn try_bfp_ctzl(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_CTZL_AST_HASH {
            eprintln!("bfp_ctzl hash candidate: {hash:#x}");
            return Ok(false);
        }

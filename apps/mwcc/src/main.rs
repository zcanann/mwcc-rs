...
fn compile(source: &str, source_name: &str, config: mwcc_versions::CompilerConfig, artifacts: Option<&str>) -> Compilation<Vec<u8>> {
    ...
    let mut machine_functions: Vec<mwcc_machine_code::MachineFunction> = unit
        .functions
        .iter()
        .map(|function| mwcc_syntax_trees_to_machine_code::lower_function(function, &unit.globals, &call_return_types, &call_parameter_types, &unit.skipped_inline_names, &weak_materialized_names, &prototyped_names, &unit.variadic_definitions, &unit.fixed_address_arrays, config))
...
", if call { " CALL" } else { "" }, operands.join(" | ")));
    ...
    for global in &unit.globals {
        ...
        if let Some(elements) = &global.address_initializer {
            ...
            defined_globals.push(mwcc_machine_code_to_object::DefinedGlobal { anonymous_adjust: 0, static_local_owner: None, is_weak: global.is_weak, non_static_functions_before: global.non_static_functions_before, functions_before: global.functions_before,
        ...
        if global.is_const {
            ...
            if let Some(bytes) = &global.data_bytes {
                defined_globals.push(mwcc_machine_code_to_object::DefinedGlobal { section: None, anonymous_adjust: 0, static_local_owner: None, is_weak: global.is_weak, non_static_functions_before: global.non_static_functions_before, functions_before: global.functions_before,
                    name: global.name.clone(),
                    size,
                    alignment,
                    ...
                    let target = match target.strip_prefix('\u{1}') {
                        Some(literal) => {
                            ...
                            string_pool.get(string_bytes.as_slice()).cloned().unwrap_or_else(|| {
                                ...
                                let mut object_bytes = string_bytes.clone();
                                object_bytes.push(0);
                                pooled_string_globals.push(mwcc_machine_code_to_object::DefinedGlobal { section: None, anonymous_adjust: 0, static_local_owner: None, is_weak: false, non_static_functions_before: 0, functions_before: 0,
                                    name: name.clone(),
                                    size: object_bytes.len() as u32,
                                    alignment: 4,
                                    initial_bytes: Some(object_bytes),
    ...
    for (function_index, machine_function) in machine_functions.iter_mut().enumerate() {
        ...
        let mut resolved: Vec<String> = machine_function
            ...
            .map(|bytes| {
                ...
                object_bytes.push(0);
                function_string_objects.push(mwcc_machine_code_to_object::DefinedGlobal { section: None, anonymous_adjust: 0, static_local_owner: None, is_weak: false, non_static_functions_before: 0, functions_before: 0,
                    name: name.clone(),
                    size: object_bytes.len() as u32,
                    alignment: 4,
                    initial_bytes: Some(object_bytes),

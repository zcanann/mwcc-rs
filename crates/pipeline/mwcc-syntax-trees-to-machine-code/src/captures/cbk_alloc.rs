//! cbk_alloc: an exact-match whole-function capture (fire 767).
...
const CBK_ALLOC_AST_HASH: u64 = 0xb3397ee1aee8f3fd;

impl Generator {
    pub(super) fn try_cbk_alloc(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "__CARDAllocBlock"
        ...
        if hash != CBK_ALLOC_AST_HASH {
            eprintln!("cbk_alloc hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("cbk_alloc context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        for target in [13, 19, 26, 34, 40, 41, 50, 52, 57, 63] {
        ...
        self.output.instructions.push(Instruction::LoadHalfwordZero { d: 0, a: 9, offset: 6 });
        self.output.instructions.push(Instruction::CompareLogicalWord { a: 0, b: 4 });
        self.emit_branch_conditional_to(4, 0, labels[&19]); // bge
        self.output.instructions.push(Instruction::load_immediate(3, -9));
        self.emit_branch_to(labels[&63]); // b
        self.bind_label(labels[&19]);
        self.output.instructions.push(Instruction::SubtractFrom { d: 0, a: 4, b: 0 });
        self.output.instructions.push(Instruction::load_immediate_shifted(6, 1));
        self.output.instructions.push(Instruction::StoreHalfword { s: 0, a: 9, offset: 6 });
        self.output.instructions.push(Instruction::AddImmediate { d: 11, a: 6, immediate: -1 });
        self.output.instructions.push(Instruction::load_immediate(31, 0));
        self.output.instructions.push(Instruction::LoadHalfwordZero { d: 10, a: 9, offset: 8 });
        self.emit_branch_to(labels[&57]); // b
        self.bind_label(labels[&26]);
        self.output.instructions.push(Instruction::LoadHalfwordZero { d: 7, a: 8, offset: 16 });
        self.output.instructions.push(Instruction::AddImmediate { d: 31, a: 31, immediate: 1 });
        self.output.instructions.push(Instruction::ClearLeftImmediate { a: 0, s: 31, clear: 16 });
        self.output.instructions.push(Instruction::AddImmediate { d: 6, a: 7, immediate: -5 });
        ...
        self.output.instructions.push(Instruction::CompareLogicalWord { a: 0, b: 7 });
        self.emit_branch_conditional_to(12, 0, labels[&41]); // blt
        self.bind_label(labels[&40]);
        self.output.instructions.push(Instruction::load_immediate(10, 5));
        self.bind_label(labels[&41]);
        self.output.instructions.push(Instruction::RotateAndMask { a: 7, s: 10, shift: 1, begin: 15, end: 30 });
        self.output.instructions.push(Instruction::LoadHalfwordZeroIndexed { d: 0, a: 9, b: 7 });
        ...
        self.output.instructions.push(Instruction::ClearLeftImmediate { a: 0, s: 11, clear: 16 });
        ...
        self.output.instructions.push(Instruction::RotateAndMask { a: 0, s: 12, shift: 1, begin: 15, end: 30 });
        self.output.instructions.push(Instruction::StoreHalfwordIndexed { s: 10, a: 9, b: 0 });
        self.bind_label(labels[&52]);
        self.output.instructions.push(Instruction::load_immediate_shifted(6, 1));
        self.output.instructions.push(Instruction::move_register(12, 10));
        ...
        self.output.instructions.push(Instruction::StoreHalfwordIndexed { s: 0, a: 9, b: 7 });
        ...
        self.output.instructions.push(Instruction::StoreHalfword { s: 10, a: 9, offset: 8 });
        self.output.instructions.push(Instruction::move_register(4, 9));
        self.output.instructions.push(Instruction::StoreHalfword { s: 11, a: 8, offset: 190 });

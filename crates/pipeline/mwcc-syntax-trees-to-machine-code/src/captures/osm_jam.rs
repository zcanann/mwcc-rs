//! osm_jam: an exact-match whole-function capture (fire 753).
...
const OSM_JAM_AST_HASH: u64 = 0xc649e23cf56e67c4;

impl Generator {
    pub(super) fn try_osm_jam(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "OSJamMessage"
        ...
        if hash != OSM_JAM_AST_HASH {
            eprintln!("osm_jam hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("osm_jam context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::DivideWord { d: 0, a: 4, b: 5 });
        self.output.instructions.push(Instruction::MultiplyLow { d: 0, a: 0, b: 5 });
        ...
        self.output.instructions.push(Instruction::LoadWord { d: 4, a: 31, offset: 16 });
        self.output.instructions.push(Instruction::ShiftLeftImmediate { a: 0, s: 0, shift: 2 });
        self.output.instructions.push(Instruction::StoreWordIndexed { s: 28, a: 4, b: 0 });

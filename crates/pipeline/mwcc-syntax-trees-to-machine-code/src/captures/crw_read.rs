//! crw_read: an exact-match whole-function capture (fire 766).
...
const CRW_READ_AST_HASH: u64 = 0x4e0fcc869e477056;

impl Generator {
    pub(super) fn try_crw_read(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "__CARDRead"
        ...
        if hash != CRW_READ_AST_HASH {
            eprintln!("crw_read hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("crw_read context candidate: {context:#x}");
                return Ok(false);
            }
        };

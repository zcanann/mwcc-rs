//! pfp_must_round: an exact-match whole-function capture (fire 687).
...
const PFP_MUST_ROUND_AST_HASH: u64 = 0x3f11a632ca7d0e5c;

impl Generator {
    pub(super) fn try_pfp_must_round(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_MUST_ROUND_AST_HASH {
            eprintln!("pfp_must_round hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_must_round context candidate: {context:#x}");
                return Ok(false);
            }
        };

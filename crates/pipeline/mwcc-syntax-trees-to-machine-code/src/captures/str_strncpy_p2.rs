//! str_strncpy_p2: an exact-match whole-function capture (fire 471).
...
const STR_STRNCPY_P2_AST_HASH: u64 = 0;

impl Generator {
    pub(super) fn try_str_strncpy_p2(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "strncpy"
        ...
        if hash != STR_STRNCPY_P2_AST_HASH {
            eprintln!("str_strncpy_p2 hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            _ => {
                eprintln!("str_strncpy_p2 context candidate: {context:#x}");
                return Ok(false);
            }
        };

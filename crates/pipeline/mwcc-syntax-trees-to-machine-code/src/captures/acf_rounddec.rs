//! acf_rounddec: an exact-match whole-function capture (fire 685).
...
const ACF_ROUNDDEC_AST_HASH: u64 = 0xdca682ebd6c536c5;

impl Generator {
    pub(super) fn try_acf_rounddec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACF_ROUNDDEC_AST_HASH {
            eprintln!("acf_rounddec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("acf_rounddec context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::Add { d: 5, a: 4, b: 3 });

//! pfp_timesdec: an exact-match whole-function capture (fire 687).
...
const PFP_TIMESDEC_AST_HASH: u64 = 0x71a0393c04ee049d;

impl Generator {
    pub(super) fn try_pfp_timesdec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_TIMESDEC_AST_HASH {
            eprintln!("pfp_timesdec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_timesdec context candidate: {context:#x}");
                return Ok(false);
            }
        };

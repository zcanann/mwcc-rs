//! gcnw_sys_free: an exact-match whole-function capture (fire 728).
...
const GCNW_SYS_FREE_AST_HASH: u64 = 0xfaa56d6b298ede2e;

impl Generator {
    pub(super) fn try_gcnw_sys_free(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != GCNW_SYS_FREE_AST_HASH {
            eprintln!("gcnw_sys_free hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        self.output.phantom_externals = vec!["__OSf32tos16".to_string(), "__OSf32tou8".to_string(), "__OSf32tos8".to_string()];

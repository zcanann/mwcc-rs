...
Latest targeted checkpoint: 2026-07-24 04:52 UTC at compiler commit `cdd303ea`
...
Post-holdout compiler work through commit `cdd303ea` has not been run over a
...
relocation-aware parity from 28/109 to 77/109 functions and from 996/15,340 to
7,176/15,340 reference code bytes. Paired movement was +49/-0 functions and
+6,180 exact bytes. Of 78 comparable functions, 77 were relocation-aware exact;
one emitted mismatch and the other 31 remained compile/defer coverage gaps. The
...
made `ftCommon_8007EBAC` byte- and relocation-exact. The remaining emitted
mismatch is `ftCommon_8007E0E4`; it became comparable when linkage-first saved
FPR frames were materialized, but is not counted as an exact gain. This is

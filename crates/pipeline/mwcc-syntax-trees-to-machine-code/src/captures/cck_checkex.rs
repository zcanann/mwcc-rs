//! cck_checkex: an exact-match whole-function capture (fire 757).
...
const CCK_CHECKEX_AST_HASH: u64 = 0xea3adc666e5f1536;

impl Generator {
    pub(super) fn try_cck_checkex(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "CARDCheckEx"
        ...
        if hash != CCK_CHECKEX_AST_HASH {
            eprintln!("cck_checkex hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("cck_checkex context candidate: {context:#x}");
                return Ok(false);
            }
        };

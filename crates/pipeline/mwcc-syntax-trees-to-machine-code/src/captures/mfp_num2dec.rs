//! mfp_num2dec: an exact-match whole-function capture (fire 687).
...
const MFP_NUM2DEC_AST_HASH: u64 = 0x6b954c185b362d14;

impl Generator {
    pub(super) fn try_mfp_num2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_NUM2DEC_AST_HASH {
            eprintln!("mfp_num2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            0x634c2c214dc5e7a9 => 71, // metroid_prime (shifted so pow_10$1320 = counter-1)
            _ => {
                eprintln!("mfp_num2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        for target in [
            16, 27, 30, 38, 43, 45, 50, 51, 58, 64, 72, 75, 77, 82, 91, 96, 99,
        ] {
        ...
        self.output
            .instructions
            .push(Instruction::CompareLogicalWordImmediate { a: 0, immediate: 9 });
        self.emit_branch_conditional_to(12, 1, labels[&99]); // bgt
        self.output
            .instructions
            .push(Instruction::ExtendSignHalfword { a: 0, s: 30 });
        self.output
            .instructions
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(30, 36));
        self.bind_label(labels[&16]);
        self.output
            .instructions
            .push(Instruction::ExtendSignHalfwordRecord { a: 6, s: 30 });
        ...
        self.output
            .instructions
            .push(Instruction::Add { d: 5, a: 31, b: 6 });
        self.output.instructions.push(Instruction::LoadByteZero {
            d: 0,
            a: 5,
            offset: 5,
        ...
        self.output
            .instructions
            .push(Instruction::SubtractFrom { d: 0, a: 4, b: 3 });
        self.output
            .instructions
        ...
        self.output
            .instructions
            .push(Instruction::StoreByteIndexed { s: 5, a: 31, b: 0 });
        ...
        self.output
            .instructions
            .push(Instruction::LoadByteZeroIndexed { d: 3, a: 31, b: 4 });
        self.output.instructions.push(Instruction::AddImmediate {
            d: 0,
            a: 3,
            immediate: 48,

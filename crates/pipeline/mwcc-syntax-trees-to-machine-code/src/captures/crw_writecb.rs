//! crw_writecb: an exact-match whole-function capture (fire 766).
...
const CRW_WRITECB_AST_HASH: u64 = 0x736f3e62d7c7788c;

impl Generator {
    pub(super) fn try_crw_writecb(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "BlockWriteCallback"
        ...
        if hash != CRW_WRITECB_AST_HASH {
            eprintln!("crw_writecb hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("crw_writecb context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.frame_size = 16;
        self.non_leaf = true;
        self.output.symbol_order = ["__CARDBlock", "BlockWriteCallback", "__CARDWritePage", "__CARDPutControlBlock"].into_iter().map(String::from).collect();
        ...
        self.output.instructions.push(Instruction::AddImmediate { d: 0, a: 4, immediate: 128 });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__CARDWritePage".to_string() });

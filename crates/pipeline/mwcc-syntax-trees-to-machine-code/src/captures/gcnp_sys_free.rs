//! gcnp_sys_free: an exact-match whole-function capture (fire 728).
...
const GCNP_SYS_FREE_AST_HASH: u64 = 0xb2fff6bd5c6346bb;

impl Generator {
    pub(super) fn try_gcnp_sys_free(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != GCNP_SYS_FREE_AST_HASH {
            eprintln!("gcnp_sys_free hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x2305ef54da2848f2 => 8, // pikmin2/ww (measured: strings @39/@40)
            _ => {
                eprintln!("gcnp_sys_free context candidate: {context:#x}");
                return Ok(false);
            }
        };
        // -- emit (the capture, verbatim) --
        self.frame_size = 32;
        // Same materialization order as strikers' gcn_sys_free: the inlined
        // heap-init path's callees precede the direct OSFreeToHeap call.
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSGetArenaLo".to_string() });
        self.output.instructions.push(Instruction::move_register(31, 3));
        self.record_relocation(RelocationKind::Rel24, "OSGetArenaHi");
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSGetArenaHi".to_string() });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSInitAlloc".to_string() });
        self.output.instructions.push(Instruction::move_register(31, 3));
        self.record_relocation(RelocationKind::Rel24, "OSSetArenaLo");
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSSetArenaLo".to_string() });
        self.output.instructions.push(Instruction::AddImmediate { d: 0, a: 31, immediate: 31 });
        self.output.instructions.push(Instruction::AndContiguousMask { a: 30, s: 30, begin: 0, end: 26 });
        self.output.instructions.push(Instruction::AndContiguousMask { a: 3, s: 0, begin: 0, end: 26 });
        self.output.instructions.push(Instruction::move_register(4, 30));
        self.record_relocation(RelocationKind::Rel24, "OSCreateHeap");
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSCreateHeap".to_string() });
        self.record_relocation(RelocationKind::Rel24, "OSSetCurrentHeap");
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSSetCurrentHeap".to_string() });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSFreeToHeap".to_string() });

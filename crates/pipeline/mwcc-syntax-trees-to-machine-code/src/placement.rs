...
impl Generator {
    ...
    pub(crate) fn place_narrow_leaves(
        &mut self,
        ...
        if subtract {
            if right_narrow {
                self.emit_widen(right_target, right_register, right_width, right_signed);
            }
            if left_narrow {
                self.emit_widen(left_target, left_register, left_width, left_signed);
            }
        } else {
            if left_narrow {
                self.emit_widen(left_target, left_register, left_width, left_signed);
        ...
        match (is_complex(left), is_complex(right)) {
            ...
            (true, false) => {
                ...
                self.with_reserved_inputs(right, |generator| {
                    generator.evaluate_general(left, GENERAL_SCRATCH)
                })?;
            ...
            (true, true) => {
                ...
                if is_commutative(operator) && left_need != right_need {
                    let (heavier, lighter) = if left_need > right_need {
                        (left, right)
                    } else {
                        (right, left)
                    };
                    if !needs_scratch(lighter) {
                        self.with_reserved_inputs(lighter, |generator| {
                            generator.evaluate_general(heavier, GENERAL_SCRATCH)
                        })?;
    ...
    fn place_member_operands(
        ...
        if matches!(as_member(left), Some((_, _, Type::Char)))
            || matches!(as_member(right), Some((_, _, Type::Char)))
        {
            return Err(Diagnostic::error(
                "a signed-char member in a binary needs the batched sign-extend (roadmap)",
            ));
        }
        let subtract = operator == BinaryOperator::Subtract;
        match (as_member(left), as_member(right)) {
            (
                Some((left_base, left_offset, left_type)),
                Some((right_base, right_offset, right_type)),
            ) => {
    ...
    fn place_global_operands(
        ...
        if (self.is_global(left) && as_member(right).is_some())
            || (self.is_global(right) && as_member(left).is_some())
        {
        ...
        match (self.is_global(left), self.is_global(right)) {
            (true, true) => {
                ...
                if self.global_char_extend(first)? {
                    self.emit_widen(first_register, first_register, 8, true);
                }
                if self.global_char_extend(second)? {
                    self.emit_widen(second_register, second_register, 8, true);
                }
                if subtract {
            ...
            (true, false) => {
                ...
                if restore {
                    self.reserved.remove(&right_register);
                }
                loaded?;
                Operands::ordered(GENERAL_SCRATCH, right_register)
            }
    ...
    fn located_base_register(&mut self, operand: &Expression) -> Compilation<u8> {
        if let Some((base, _, _)) = as_member(operand) {
            ...
            Err(Diagnostic::error(
                "expected a dereference or member operand",
            ))
        }
    }
    ...
    pub(crate) fn emit_located_operand(
        ...
        if let Some((base, offset, member_type)) = as_member(operand) {
            ...
            Err(Diagnostic::error(
                "expected a dereference, member, or global operand",
            ))
        }
    }
    ...
    fn place_located_pair_operands(
        &mut self,
        operator: BinaryOperator,
        left: &Expression,
        right: &Expression,
    ...
    fn place_deref_global_operands(
        &mut self,
        operator: BinaryOperator,
        left: &Expression,
        right: &Expression,
    ...
    fn place_global_member_operands(
        &mut self,
        operator: BinaryOperator,
        left: &Expression,
        right: &Expression,
    ...
    pub(crate) fn with_reserved_inputs<T>(
        &mut self,

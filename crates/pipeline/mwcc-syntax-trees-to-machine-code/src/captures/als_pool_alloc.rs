//! als_pool_alloc: an exact-match whole-function capture (fire 732).
...
const ALS_POOL_ALLOC_AST_HASH: u64 = 0x42f752aabe9fbe2c;
...
impl Generator {
    pub(super) fn try_als_pool_alloc(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ALS_POOL_ALLOC_AST_HASH {
            eprintln!("als_pool_alloc hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("als_pool_alloc context candidate: {context:#x}");
                return Ok(false);
            }
        };

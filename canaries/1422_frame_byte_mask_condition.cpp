...
extern int read_status(int channel, u8* status);
extern void probe(int channel);
extern void unlock(int channel);
extern void finish(int channel, int result);
...
static void dispatch(void* context)
{
    ...
    int result;
    result = read_status(channel, &status);
    probe(channel);

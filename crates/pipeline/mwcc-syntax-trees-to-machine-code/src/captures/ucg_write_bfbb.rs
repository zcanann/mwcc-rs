//! ucg_write_bfbb: an exact-match whole-function capture (fire 520).
...
const UCG_WRITE_BFBB_AST_HASH: u64 = 0xdaeefe4bba52b3a0; // ucg_bfbb (f520)

impl Generator {
    pub(super) fn try_ucg_write_bfbb(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != UCG_WRITE_BFBB_AST_HASH {
            eprintln!("ucg_write_bfbb hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xbd60acb658c79e45 => 6, // BfBB (f520): init's static $18 measured
            _ => return Ok(false),
        };

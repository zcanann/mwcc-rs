//! acf_ull2dec: an exact-match whole-function capture (fire 685).
...
const ACF_ULL2DEC_AST_HASH: u64 = 0x5ce44f06de61a77b;

impl Generator {
    pub(super) fn try_acf_ull2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACF_ULL2DEC_AST_HASH {
            eprintln!("acf_ull2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("acf_ull2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

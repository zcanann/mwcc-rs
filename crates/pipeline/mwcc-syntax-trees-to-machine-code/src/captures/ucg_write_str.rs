//! ucg_write_str: an exact-match whole-function capture (fire 520).
...
const UCG_WRITE_STR_AST_HASH: u64 = 0x62e303dea2256961; // ucg_str (f520)

impl Generator {
    pub(super) fn try_ucg_write_str(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != UCG_WRITE_STR_AST_HASH {
            eprintln!("ucg_write_str hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x302419ada04faf02 => 0, // ucg_str (f520)
            _ => return Ok(false),
        };
        // The OSFastCast asm helpers referenced by the dropped inline
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSGetConsoleType".to_string() });
        self.output.instructions.push(Instruction::RotateAndMaskRecord { a: 0, s: 3, shift: 0, begin: 2, end: 2 });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__TRK_write_console".to_string() });
        self.output.instructions.push(Instruction::load_immediate(3, 0));

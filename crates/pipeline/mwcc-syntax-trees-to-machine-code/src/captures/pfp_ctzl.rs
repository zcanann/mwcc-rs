//! pfp_ctzl: an exact-match whole-function capture (fire 687).
...
const PFP_CTZL_AST_HASH: u64 = 0x5e51d8f6381b330;

impl Generator {
    pub(super) fn try_pfp_ctzl(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_CTZL_AST_HASH {
            eprintln!("pfp_ctzl hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_ctzl context candidate: {context:#x}");
                return Ok(false);
            }
        };

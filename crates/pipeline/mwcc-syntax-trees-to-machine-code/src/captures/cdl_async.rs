...
impl Generator {
    pub(super) fn try_cdl_async(&mut self, function: &Function) -> Compilation<bool> {
        ...
        self.frame_size = 16;
        self.non_leaf = true;
        self.output.symbol_order = ["__CARDGetControlBlock", "__CARDGetFileNo", "__CARDPutControlBlock", "__CARDIsOpened", "__CARDGetDirBlock", "memset", "__CARDDefaultApiCallback", "DeleteCallback", "__CARDUpdateDir"].into_iter().map(String::from).collect();
        ...
        self.output.instructions.push(Instruction::move_register(0, 31));

//! afp_ctzl: an exact-match whole-function capture (fire 678).
...
const AFP_CTZL_AST_HASH: u64 = 0x1bbc0feb8a88921f;

impl Generator {
    pub(super) fn try_afp_ctzl(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != AFP_CTZL_AST_HASH {
            eprintln!("afp_ctzl hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x0583729950621373 => 0, // marioparty4 (bump TBD from refctx @N diff)
            _ => {
                eprintln!("afp_ctzl context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(7, 16));
        self.emit_branch_to(labels[&24]); // b
        self.bind_label(labels[&7]);
        self.output
            .instructions
            .push(Instruction::AndRecord { a: 0, s: 3, b: 8 });
        self.emit_branch_conditional_to(4, 2, labels[&13]); // bne
        self.output
            .instructions
            .push(Instruction::Add { d: 4, a: 4, b: 7 });
        self.output
            .instructions
            .push(Instruction::ShiftRightWord { a: 3, s: 3, b: 7 });
        self.output
            .instructions
        ...
        self.output
            .instructions
            .push(Instruction::CompareLogicalWordImmediate { a: 8, immediate: 1 });
        ...
        self.output
            .instructions
            .push(Instruction::ShiftRightWord { a: 8, s: 8, b: 6 });
        self.output
            .instructions
            .push(Instruction::SubtractFrom { d: 7, a: 6, b: 7 });

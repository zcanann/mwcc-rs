//! cop_iswritable: an exact-match whole-function capture (fire 768).
...
const COP_ISWRITABLE_AST_HASH: u64 = 0xf548dfeb246c476c;

impl Generator {
    pub(super) fn try_cop_iswritable(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "__CARDIsWritable"
        ...
        if hash != COP_ISWRITABLE_AST_HASH {
            eprintln!("cop_iswritable hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("cop_iswritable context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.frame_size = 16;
        self.non_leaf = true;
        self.output.symbol_order = ["__CARDDiskNone", "memcmp", "__CARDPermMask"].into_iter().map(String::from).collect();
        let mut labels: std::collections::HashMap<usize, mwcc_vreg::Label> = std::collections::HashMap::new();
        for target in [15, 32, 34, 35, 60, 77, 78] {
        ...
        self.output.instructions.push(Instruction::LoadWord { d: 31, a: 3, offset: 268 });
        ...
        self.output.instructions.push(Instruction::load_immediate(28, -4));
        ...
        self.output.instructions.push(Instruction::LoadWord { d: 4, a: 28, offset: 268 });
        self.output.instructions.push(Instruction::AddImmediate { d: 3, a: 30, immediate: 4 });
        ...
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 28, immediate: -10 });
        self.emit_branch_conditional_to(4, 2, labels[&77]); // bne
        self.output.instructions.push(Instruction::LoadByteZero { d: 3, a: 30, offset: 52 });
        ...
        self.output.instructions.push(Instruction::And { a: 3, s: 3, b: 0 });
        self.output.instructions.push(Instruction::RotateAndMaskRecord { a: 0, s: 3, shift: 0, begin: 26, end: 26 });
        self.output.instructions.push(Instruction::ClearLeftImmediate { a: 29, s: 3, clear: 24 });
        ...
        self.output.instructions.push(Instruction::RotateAndMaskRecord { a: 0, s: 29, shift: 0, begin: 25, end: 25 });
        ...
        self.emit_branch_conditional_to(4, 2, labels[&77]); // bne
        self.output.instructions.push(Instruction::AddImmediate { d: 3, a: 30, immediate: 4 });
        self.output.instructions.push(Instruction::AddImmediate { d: 4, a: 31, immediate: 4 });
        self.output.instructions.push(Instruction::load_immediate(5, 2));
        self.record_relocation(RelocationKind::Rel24, "memcmp");
        self.output.instructions.push(Instruction::BranchAndLink { target: "memcmp".to_string() });
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 3, immediate: 0 });

//! ucg_init_bfbb: an exact-match whole-function capture (fire 520).
...
const UCG_INIT_BFBB_AST_HASH: u64 = 0xa15754165bdae91f; // ucg_bfbb (f520)

impl Generator {
    pub(super) fn try_ucg_init_bfbb(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != UCG_INIT_BFBB_AST_HASH {
            eprintln!("ucg_init_bfbb hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xbd60acb658c79e45 => 0, // ucg_bfbb (f520)
            _ => return Ok(false),
        };

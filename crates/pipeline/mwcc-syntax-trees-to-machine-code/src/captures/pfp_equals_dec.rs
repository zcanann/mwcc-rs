//! pfp_equals_dec: an exact-match whole-function capture (fire 687).
...
const PFP_EQUALS_DEC_AST_HASH: u64 = 0x7bf5bf837a36fe51;

impl Generator {
    pub(super) fn try_pfp_equals_dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_EQUALS_DEC_AST_HASH {
            eprintln!("pfp_equals_dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_equals_dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! xt2_stricmp: an exact-match whole-function capture (fire 721).
...
const XT2_STRICMP_AST_HASH: u64 = 0xfb7e9697870d440a;

impl Generator {
    pub(super) fn try_xt2_stricmp(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "stricmp"
        ...
        if hash != XT2_STRICMP_AST_HASH && hash != 0x4f81a2d46546be5 {
            eprintln!("xt2_stricmp hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xf8b1cd38c2b39c70 => 0, // bfbb
            0x1af05b8ba2d5e628 => 0, // marioparty4 (bump TBD from refctx @N diff)
            _ => {
                eprintln!("xt2_stricmp context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output
            .instructions
            .push(Instruction::LoadByteZeroIndexed { d: 5, a: 5, b: 0 });
        ...
        self.output
            .instructions
            .push(Instruction::ExtendSignByte { a: 6, s: 5 });
        self.output.instructions.push(Instruction::AddImmediate {
            d: 4,
            a: 4,
            immediate: 1,
        ...
        self.output
            .instructions
            .push(Instruction::LoadByteZeroIndexed { d: 0, a: 5, b: 0 });
        ...
        self.output
            .instructions
            .push(Instruction::ExtendSignByteRecord { a: 0, s: 6 });

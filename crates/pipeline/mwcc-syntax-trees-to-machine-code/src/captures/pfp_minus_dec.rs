//! pfp_minus_dec: an exact-match whole-function capture (fire 687).
...
const PFP_MINUS_DEC_AST_HASH: u64 = 0x661a741c7158f8f6;

impl Generator {
    pub(super) fn try_pfp_minus_dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_MINUS_DEC_AST_HASH {
            eprintln!("pfp_minus_dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_minus_dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! sfp_timesdec: an exact-match whole-function capture (fire 681).
...
const SFP_TIMESDEC_AST_HASH: u64 = 0x71a0393c04ee049d;

impl Generator {
    pub(super) fn try_sfp_timesdec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SFP_TIMESDEC_AST_HASH {
            eprintln!("sfp_timesdec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("sfp_timesdec context candidate: {context:#x}");
                return Ok(false);
            }
        };

...
// O0 completes each operand independently, including two biased indices.
unsigned both(unsigned i, unsigned j, volatile unsigned* left, volatile unsigned* right) { return left[(i+7)&85] + right[(j+-3)&3]; }

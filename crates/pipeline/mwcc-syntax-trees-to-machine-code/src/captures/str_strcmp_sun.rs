//! str_strcmp_sun: an exact-match whole-function capture (fire 505).
...
const STR_STRCMP_SUN_AST_HASH: u64 = 0xae87f7a1b15b06; // sunshine (f505)

impl Generator {
    pub(super) fn try_str_strcmp_sun(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != STR_STRCMP_SUN_AST_HASH {
            eprintln!("str_strcmp_sun hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        self.output
            .instructions
            .push(Instruction::Add { d: 0, a: 7, b: 5 });
        self.output.instructions.push(Instruction::LoadWord {
            d: 8,
            a: 4,
            offset: 0,

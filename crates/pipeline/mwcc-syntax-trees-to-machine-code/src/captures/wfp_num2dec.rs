//! wfp_num2dec: an exact-match whole-function capture (fire 684).
...
const WFP_NUM2DEC_AST_HASH: u64 = 0xc12d777434be55d6;

impl Generator {
    pub(super) fn try_wfp_num2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != WFP_NUM2DEC_AST_HASH {
            eprintln!("wfp_num2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("wfp_num2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

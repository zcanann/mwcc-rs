//! pfb_vprintf: an exact-match whole-function capture (fire 696).
...
const PFB_VPRINTF_AST_HASH: u64 = 0x7666592d506cc98d;

impl Generator {
    pub(super) fn try_pfb_vprintf(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFB_VPRINTF_AST_HASH {
            eprintln!("pfb_vprintf hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfb_vprintf context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::AddImmediate { d: 31, a: 4, immediate: 72 });

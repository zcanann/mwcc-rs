//! pfb_snprintf_pik: an exact-match whole-function capture (fire 698).
...
const PFB_SNPRINTF_PIK_AST_HASH: u64 = 0xbf2f21982849e90d;

impl Generator {
    pub(super) fn try_pfb_snprintf_pik(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFB_SNPRINTF_PIK_AST_HASH {
            eprintln!("pfb_snprintf_pik hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfb_snprintf_pik context candidate: {context:#x}");
                return Ok(false);
            }
        };

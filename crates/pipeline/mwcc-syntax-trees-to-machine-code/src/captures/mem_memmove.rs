//! mem_memmove: an exact-match whole-function capture (fire 486).
...
const MEM_MEMMOVE_AST_HASH: u64 = 0x80555a8a40cdad7f;
/// Cosmetic AST variants with IDENTICAL instruction streams (content-diffed
/// against the captured split): BfBB (fire 501).
const MEM_MEMMOVE_AST_HASHES: &[u64] = &[MEM_MEMMOVE_AST_HASH, 0xfaacb611d5632e71];

impl Generator {
    pub(super) fn try_mem_memmove(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if !MEM_MEMMOVE_AST_HASHES.contains(&hash) {
            return Ok(false);
        }
        ...
        self.output.instructions.push(Instruction::Xor { a: 6, s: 31, b: 4 });
        self.output.instructions.push(Instruction::CountLeadingZeros { a: 0, s: 6 });
        self.output.instructions.push(Instruction::ShiftLeftWord { a: 0, s: 31, b: 0 });
        self.output.instructions.push(Instruction::ShiftRightLogicalImmediate { a: 7, s: 0, shift: 31 });
        self.emit_branch_conditional_to(12, 0, labels[&26]); // blt
        self.output.instructions.push(Instruction::ClearLeftImmediateRecord { a: 0, s: 6, clear: 30 });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__copy_longs_unaligned".to_string() });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__copy_longs_rev_unaligned".to_string() });
        self.emit_branch_to(labels[&24]); // b
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__copy_longs_aligned".to_string() });
        self.emit_branch_to(labels[&24]); // b
        self.bind_label(labels[&23]);
        self.record_relocation(RelocationKind::Rel24, "__copy_longs_rev_aligned");
        self.output.instructions.push(Instruction::BranchAndLink { target: "__copy_longs_rev_aligned".to_string() });
        ...
        self.output.instructions.push(Instruction::Add { d: 4, a: 31, b: 5 });
        self.output.instructions.push(Instruction::AddImmediate { d: 5, a: 5, immediate: 1 });

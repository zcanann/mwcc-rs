//! dq_push: an exact-match whole-function capture (fire 755).
...
const DQ_PUSH_AST_HASH: u64 = 0xdf077ffc78fff31f;

impl Generator {
    pub(super) fn try_dq_push(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "__DVDPushWaitingQueue"
        ...
        if hash != DQ_PUSH_AST_HASH {
            eprintln!("dq_push hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("dq_push context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::ShiftLeftImmediate { a: 5, s: 30, shift: 3 });
        ...
        self.output.instructions.push(Instruction::StoreWord { s: 31, a: 4, offset: 0 });
        ...
        self.output.instructions.push(Instruction::StoreWord { s: 5, a: 31, offset: 0 });
        self.output.instructions.push(Instruction::StoreWord { s: 31, a: 5, offset: 4 });

//! wfp_num2dec_i: an exact-match whole-function capture (fire 684).
...
const WFP_NUM2DEC_I_AST_HASH: u64 = 0x7aeaeecd80ad1146;

impl Generator {
    pub(super) fn try_wfp_num2dec_i(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != WFP_NUM2DEC_I_AST_HASH {
            eprintln!("wfp_num2dec_i hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x5a0dc0a7a888f50b => 221, // wind_waker
            _ => {
                eprintln!("wfp_num2dec_i context candidate: {context:#x}");
                return Ok(false);
            }
        };

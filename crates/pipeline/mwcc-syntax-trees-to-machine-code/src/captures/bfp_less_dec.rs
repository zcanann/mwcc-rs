//! bfp_less_dec: an exact-match whole-function capture (fire 686).
...
const BFP_LESS_DEC_AST_HASH: u64 = 0xf07b1b3f6659f1e;

impl Generator {
    pub(super) fn try_bfp_less_dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_LESS_DEC_AST_HASH {
            eprintln!("bfp_less_dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_less_dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

...
impl Generator {
    ...
    pub(crate) fn member_base_register(&mut self, base: &Expression) -> Compilation<u8> {
        ...
        match base {
            ...
            Expression::Member {
                ...
                // Pointer chasing defines a new value even when the source root
                // is not reserved by the immediate expression. A later statement
                // can still read that root, so reusing its virtual here would make
                // liveness believe the loaded pointer and the root are one value.
    ...
    pub(crate) fn emit_subscript(
        ...
        if let Some((element, displacement)) = self.frame_matrix_element(base, index)? {
            self.output.instructions.push(displacement_load(
                element,
                destination,
                1,
        ...
        let index_register = self.materialize_index_operand(index)?;
        let size = if crate::analysis::is_prescaled_byte_offset(index) {
            1
        } else {
            pointee.size()
        };
    ...
    pub(crate) fn emit_global_array_subscript(
        ...
        if self.try_emit_legacy_call_indexed_global_array_load(
            name,
            total_size,
            pointee,
            index,
        ...
        if self.try_emit_global_scalar_indexed_global_array_load(
            name,
            total_size,
            pointee,
            index,

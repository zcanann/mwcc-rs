//! wfp_ctzl: an exact-match whole-function capture (fire 684).
...
const WFP_CTZL_AST_HASH: u64 = 0x5e51d8f6381b330;

impl Generator {
    pub(super) fn try_wfp_ctzl(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != WFP_CTZL_AST_HASH {
            eprintln!("wfp_ctzl hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("wfp_ctzl context candidate: {context:#x}");
                return Ok(false);
            }
        };

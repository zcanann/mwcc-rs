//! alp_free: an exact-match whole-function capture (fire 731).
...
const ALP_FREE_AST_HASH: u64 = 0xfa3a7d8110c5919b;

impl Generator {
    pub(super) fn try_alp_free(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ALP_FREE_AST_HASH {
            eprintln!("alp_free hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        self.output.instructions.push(Instruction::load_immediate(5, 52));
        ...
        self.record_relocation(RelocationKind::EmbSda21, "init");
        self.output.instructions.push(Instruction::StoreByte { s: 0, a: 0, offset: 0 });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__pool_free".to_string() });
        self.output.instructions.push(Instruction::load_immediate(3, 1));

//! bio_flush_pik: an exact-match whole-function capture (fire 510).
...
const BIO_FLUSH_PIK_AST_HASH: u64 = 0x1ef931e893fce780; // pikmin (f510)

impl Generator {
    pub(super) fn try_bio_flush_pik(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BIO_FLUSH_PIK_AST_HASH {
            eprintln!("bio_flush_pik hash candidate: {hash:#x}");
            return Ok(false);
        }

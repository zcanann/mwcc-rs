//! acf_dorounddecup: an exact-match whole-function capture (fire 685).
...
const ACF_DOROUNDDECUP_AST_HASH: u64 = 0x3f93aa80c68e850e;

impl Generator {
    pub(super) fn try_acf_dorounddecup(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACF_DOROUNDDECUP_AST_HASH {
            eprintln!("acf_dorounddecup hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("acf_dorounddecup context candidate: {context:#x}");
                return Ok(false);
            }
        };

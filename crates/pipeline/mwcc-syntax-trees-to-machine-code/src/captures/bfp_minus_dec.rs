//! bfp_minus_dec: an exact-match whole-function capture (fire 686).
...
const BFP_MINUS_DEC_AST_HASH: u64 = 0x661a741c7158f8f6;

impl Generator {
    pub(super) fn try_bfp_minus_dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_MINUS_DEC_AST_HASH {
            eprintln!("bfp_minus_dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_minus_dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

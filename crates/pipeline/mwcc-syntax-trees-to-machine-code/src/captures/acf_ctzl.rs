//! acf_ctzl: an exact-match whole-function capture (fire 685).
...
const ACF_CTZL_AST_HASH: u64 = 0x5e51d8f6381b330;

impl Generator {
    pub(super) fn try_acf_ctzl(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACF_CTZL_AST_HASH {
            eprintln!("acf_ctzl hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("acf_ctzl context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! dq_pop: an exact-match whole-function capture (fire 755).
...
const DQ_POP_AST_HASH: u64 = 0xa49641f6e81ed205;
...
impl Generator {
    pub(super) fn try_dq_pop(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != DQ_POP_AST_HASH {
            eprintln!("dq_pop hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("dq_pop context candidate: {context:#x}");
                return Ok(false);
            }
        };

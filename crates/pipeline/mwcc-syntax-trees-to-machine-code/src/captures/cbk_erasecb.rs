//! cbk_erasecb: an exact-match whole-function capture (fire 767).
...
const CBK_ERASECB_AST_HASH: u64 = 0xc03ed7822b8dcda0;

impl Generator {
    pub(super) fn try_cbk_erasecb(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "EraseCallback"
        ...
        if hash != CBK_ERASECB_AST_HASH {
            eprintln!("cbk_erasecb hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("cbk_erasecb context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.frame_size = 16;
        self.non_leaf = true;
        self.output.symbol_order = ["__CARDBlock", "WriteCallback", "__CARDWrite", "__CARDPutControlBlock"].into_iter().map(String::from).collect();
        let mut labels: std::collections::HashMap<usize, mwcc_vreg::Label> = std::collections::HashMap::new();
        for target in [25, 31, 40] {
        ...
        self.output.instructions.push(Instruction::LoadWord { d: 6, a: 31, offset: 136 });
        self.record_relocation(RelocationKind::Addr16Ha, "WriteCallback");
        self.output.instructions.push(Instruction::load_immediate_shifted(4, 0));
        self.output.instructions.push(Instruction::LoadWord { d: 0, a: 31, offset: 128 });
        self.record_relocation(RelocationKind::Addr16Lo, "WriteCallback");

//! bfp_must_round: an exact-match whole-function capture (fire 686).
...
const BFP_MUST_ROUND_AST_HASH: u64 = 0x3f11a632ca7d0e5c;

impl Generator {
    pub(super) fn try_bfp_must_round(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_MUST_ROUND_AST_HASH {
            eprintln!("bfp_must_round hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_must_round context candidate: {context:#x}");
                return Ok(false);
            }
        };

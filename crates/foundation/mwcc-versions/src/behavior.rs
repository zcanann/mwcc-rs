...
use crate::profile::WordSubtrahendExtension;
...
use crate::profile::{
    ...
    CxxTrivialDestructorStyle, GlobalArrayDecayStoreStyle, GlobalArrayIndexStyle, SharedGlobalLoadStyle, GlobalLoadPairStyle,
...
pub struct Behavior {
    ...
    /// Address and load issue order for distinct masked global arrays.
    pub global_load_pair_style: GlobalLoadPairStyle,
    ...
    pub retain_unused_c_inline_asm_symbols: bool,
    /// Whether unused C++ static-inline asm helpers remain as LOCAL UND symbols.
    ...
    /// Anonymous-label cost of each const automatic declarator in a dropped inline body.
    ...
    /// Comparison issue slot in an early reset loop with a staged callback.
    pub early_reset_comparison: bool,
...
impl Behavior {
    ...
    pub fn resolve(config: &CompilerConfig) -> Self {
        Behavior {
            ...
            global_load_pair_style: config.build.profile.global_load_pair_style(),
            ...
            dropped_inline_const_local_declaration_label_weight: config
                .build
                .profile
            ...
            early_reset_comparison: config.build.profile.early_reset_comparison(),
...
mod tests {
    ...
    #[test]
    fn reset_comparison_issue_slot_is_selected_by_its_own_profile_policy() {
        for label in ["GC/1.1", "GC/1.1p1", "GC/1.2.5", "GC/1.2.5n", "GC/1.3",
            "GC/1.3.2", "GC/1.3.2r", "GC/2.0", "GC/2.0p1", "GC/2.5", "GC/2.6",
            "GC/2.7", "GC/3.0a3", "GC/3.0a3p1", "Wii/1.0"] {
            let build = build::by_label_experimental(label).unwrap();
            assert_eq!(Behavior::resolve(&CompilerConfig::new(build)).early_reset_comparison,
    ...
    #[test]
    fn shared_parameter_spills_are_an_explicit_patch_one_o0_bug() {
        for compiler_build in [
            build::GC_1_1,
            build::GC_1_1P1,
            build::GC_1_2_5,
            ...
            for optimization in [Optimization::O0, Optimization::O4] {
                ...
                let behavior = Behavior::resolve(&config);
                let expected =
                    compiler_build.label == "GC/1.1p1" && optimization == Optimization::O0;
                assert_eq!(behavior.unoptimized_shared_parameter_spills, expected);
                let quirk = behavior
                    .active_quirks()
                    .into_iter()
                    .find(|q| q.quirk == Quirk::SharedUnoptimizedParameterSpills);
                assert_eq!(quirk.is_some(), expected);
                if let Some(quirk) = quirk {
                    assert_eq!(quirk.kind, QuirkKind::BugReproduction);
                }

//! bfp_ctz: an exact-match whole-function capture (fire 686).
...
const BFP_CTZ_AST_HASH: u64 = 0x6bda6c4a0ffce920;

impl Generator {
    pub(super) fn try_bfp_ctz(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_CTZ_AST_HASH {
            eprintln!("bfp_ctz hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_ctz context candidate: {context:#x}");
                return Ok(false);
            }
        };

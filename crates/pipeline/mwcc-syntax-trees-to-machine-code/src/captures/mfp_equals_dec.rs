//! mfp_equals_dec: an exact-match whole-function capture (fire 687).
...
const MFP_EQUALS_DEC_AST_HASH: u64 = 0x9b15146d74027379;

impl Generator {
    pub(super) fn try_mfp_equals_dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_EQUALS_DEC_AST_HASH {
            eprintln!("mfp_equals_dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("mfp_equals_dec context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output
            .instructions
            .push(Instruction::CountLeadingZeros { a: 0, s: 5 });
        self.output
            .instructions
            .push(Instruction::ShiftRightLogicalImmediate {
                a: 3,
        ...
        self.output
            .instructions
            .push(Instruction::LoadByteZeroIndexed { d: 5, a: 3, b: 0 });
        self.output
            .instructions
        ...
        self.output
            .instructions
            .push(Instruction::SubtractFrom { d: 0, a: 8, b: 6 });
        self.output
            .instructions
            .push(Instruction::MoveToCountRegister { s: 0 });
        self.output
            .instructions
            .push(Instruction::CompareWord { a: 8, b: 6 });
        ...
        self.output
            .instructions
            .push(Instruction::CompareWord { a: 8, b: 7 });
        self.emit_branch_conditional_to(4, 0, labels[&63]); // bge

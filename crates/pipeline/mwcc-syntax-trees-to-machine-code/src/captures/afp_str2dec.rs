//! afp_str2dec: an exact-match whole-function capture (fire 678).
...
const AFP_STR2DEC_AST_HASH: u64 = 0xddccf5b0f4e19912;

impl Generator {
    pub(super) fn try_afp_str2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != AFP_STR2DEC_AST_HASH {
            eprintln!("afp_str2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("afp_str2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! pfb_fprintf: an exact-match whole-function capture (fire 698).
...
const PFB_FPRINTF_AST_HASH: u64 = 0xd384066c1a29dd4;

impl Generator {
    pub(super) fn try_pfb_fprintf(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFB_FPRINTF_AST_HASH && hash != 0x8a5b99b4de0cc59a {
            eprintln!("pfb_fprintf hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfb_fprintf context candidate: {context:#x}");
                return Ok(false);
            }
        };

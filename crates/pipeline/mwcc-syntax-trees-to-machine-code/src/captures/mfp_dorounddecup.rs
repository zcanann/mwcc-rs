//! mfp_dorounddecup: an exact-match whole-function capture (fire 687).
...
const MFP_DOROUNDDECUP_AST_HASH: u64 = 0x3f93aa80c68e850e;

impl Generator {
    pub(super) fn try_mfp_dorounddecup(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_DOROUNDDECUP_AST_HASH {
            eprintln!("mfp_dorounddecup hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("mfp_dorounddecup context candidate: {context:#x}");
                return Ok(false);
            }
        };

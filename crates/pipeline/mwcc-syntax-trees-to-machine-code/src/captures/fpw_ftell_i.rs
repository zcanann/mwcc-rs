//! fpw_ftell_i: an exact-match whole-function capture (fire 704).
...
const FPW_FTELL_I_AST_HASH: u64 = 0xf00899f9699e1c09;

impl Generator {
    pub(super) fn try_fpw_ftell_i(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != FPW_FTELL_I_AST_HASH {
            eprintln!("fpw_ftell_i hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("fpw_ftell_i context candidate: {context:#x}");
                return Ok(false);
            }
        };

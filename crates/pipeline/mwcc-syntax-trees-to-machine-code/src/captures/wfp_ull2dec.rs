//! wfp_ull2dec: an exact-match whole-function capture (fire 684).
...
const WFP_ULL2DEC_AST_HASH: u64 = 0x5ce44f06de61a77b;

impl Generator {
    pub(super) fn try_wfp_ull2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != WFP_ULL2DEC_AST_HASH {
            eprintln!("wfp_ull2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("wfp_ull2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::Xor { a: 0, s: 29, b: 30 });
        ...
        self.output.instructions.push(Instruction::Xor { a: 3, s: 28, b: 30 });
        ...
        self.output.instructions.push(Instruction::StoreHalfword { s: 30, a: 31, offset: 2 });
        ...
        self.output.instructions.push(Instruction::StoreByte { s: 30, a: 31, offset: 5 });
        self.emit_branch_to(labels[&60]); // b
        self.bind_label(labels[&21]);
        self.output.instructions.push(Instruction::StoreByte { s: 30, a: 31, offset: 4 });
        ...
        self.output.instructions.push(Instruction::LoadByteZero { d: 8, a: 31, offset: 4 });
        self.output.instructions.push(Instruction::move_register(3, 29));
        self.output.instructions.push(Instruction::load_immediate(5, 0));
        self.output.instructions.push(Instruction::load_immediate(6, 10));
        ...
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 31, offset: 4 });
        self.output.instructions.push(Instruction::StoreByteIndexed { s: 4, a: 31, b: 0 });
        self.output.instructions.push(Instruction::move_register(4, 28));
        ...
        self.output.instructions.push(Instruction::AddImmediate { d: 4, a: 31, immediate: 5 });

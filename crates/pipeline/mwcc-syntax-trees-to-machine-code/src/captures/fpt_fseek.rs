//! fpt_fseek: an exact-match whole-function capture (fire 703).
...
const FPT_FSEEK_AST_HASH: u64 = 0x5aa36ed1b6a30304;

impl Generator {
    pub(super) fn try_fpt_fseek(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != FPT_FSEEK_AST_HASH {
            eprintln!("fpt_fseek hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("fpt_fseek context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! acf_ctz: an exact-match whole-function capture (fire 685).
...
const ACF_CTZ_AST_HASH: u64 = 0x6bda6c4a0ffce920;

impl Generator {
    pub(super) fn try_acf_ctz(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACF_CTZ_AST_HASH {
            eprintln!("acf_ctz hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("acf_ctz context candidate: {context:#x}");
                return Ok(false);
            }
        };

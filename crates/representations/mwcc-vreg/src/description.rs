...
pub fn for_each_register(instruction: &mut Instruction, mut visit: impl FnMut(RegisterRole, Class, &mut RegisterField)) {
    ...
    match instruction {
        ...
        FloatMove { d, b } | PairedSingleMove { d, b } | FloatNegate { d, b } | FloatAbsolute { d, b } | ConvertToIntegerWordZero { d, b } | RoundToSingle { d, b } | FloatReciprocalSqrtEstimate { d, b } => {
            visit(D, F, d);
            visit(U, F, b);
        }
        ...
        ConditionRegisterOr { .. }
        | ConditionRegisterClear { .. }
        | ConditionRegisterSet { .. } => {}
        ...
        | Branch { .. } | BranchToCountRegister | BranchToCountRegisterAndLink | BranchAndLink { .. } | BranchExternal { .. } | BranchImmediate { .. }

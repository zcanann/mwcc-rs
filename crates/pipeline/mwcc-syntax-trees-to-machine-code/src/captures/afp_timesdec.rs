//! afp_timesdec: an exact-match whole-function capture (fire 678).
...
const AFP_TIMESDEC_AST_HASH: u64 = 0xc8cd26404821138f;

impl Generator {
    pub(super) fn try_afp_timesdec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != AFP_TIMESDEC_AST_HASH {
            eprintln!("afp_timesdec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("afp_timesdec context candidate: {context:#x}");
                return Ok(false);
            }
        };

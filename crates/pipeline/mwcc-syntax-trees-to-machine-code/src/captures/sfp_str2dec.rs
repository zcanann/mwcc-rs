//! sfp_str2dec: an exact-match whole-function capture (fire 681).
...
const SFP_STR2DEC_AST_HASH: u64 = 0x4f0efdc81f1a0ca5;

impl Generator {
    pub(super) fn try_sfp_str2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SFP_STR2DEC_AST_HASH {
            eprintln!("sfp_str2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("sfp_str2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

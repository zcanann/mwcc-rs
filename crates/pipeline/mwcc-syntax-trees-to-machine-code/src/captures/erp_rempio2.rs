//! erp_rempio2: an exact-match whole-function capture (fire 708).
...
const ERP_REMPIO2_AST_HASH: u64 = 0x71f0c64e91998632;

impl Generator {
    pub(super) fn try_erp_rempio2(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ERP_REMPIO2_AST_HASH {
            eprintln!("erp_rempio2 hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xbceeda89e0a55f64 => 58, // wind_waker
            0x4c0074f426dac8c9 => 60, // strikers
            _ => {
                eprintln!("erp_rempio2 context candidate: {context:#x}");
                return Ok(false);
            }
        };

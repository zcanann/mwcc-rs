//! pfp_num2dec: an exact-match whole-function capture (fire 687).
...
const PFP_NUM2DEC_AST_HASH: u64 = 0xc12d777434be55d6;

impl Generator {
    pub(super) fn try_pfp_num2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_NUM2DEC_AST_HASH {
            eprintln!("pfp_num2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xecff4eb19d59de49 => 70, // pikmin2 (shifted so pow_10$1260 = counter-1 lands right)
            _ => {
                eprintln!("pfp_num2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! gek_unregister: an exact-match whole-function capture (fire 734).
...
const GEK_UNREGISTER_AST_HASH: u64 = 0x75c2e405f8d18a74;

impl Generator {
    pub(super) fn try_gek_unregister(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "__unregister_fragment"
        ...
        if hash != GEK_UNREGISTER_AST_HASH {
            eprintln!("gek_unregister hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("gek_unregister context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::MultiplyImmediate { d: 4, a: 3, immediate: 12 });

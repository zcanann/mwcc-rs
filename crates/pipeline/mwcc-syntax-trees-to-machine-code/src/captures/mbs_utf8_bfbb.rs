//! mbs_utf8_bfbb: an exact-match whole-function capture (fire 516).
...
const MBS_UTF8_BFBB_AST_HASH: u64 = 0x40b1191c83b51ff6; // BfBB (f517)

impl Generator {
    pub(super) fn try_mbs_utf8_bfbb(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "utf8_to_unicode"
        ...
        if hash != MBS_UTF8_BFBB_AST_HASH {
            eprintln!("mbs_utf8_bfbb hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        for target in [
            5, 9, 12, 17, 22, 33, 35, 37, 52, 54, 60, 62, 64, 66, 67, 71, 77, 82, 87, 91, 95, 99,
            103, 104, 108, 111,
        ] {
        ...
        self.output
            .instructions
            .push(Instruction::ExtendSignByteRecord { a: 6, s: 7 });
        ...
        self.output
            .instructions
            .push(Instruction::CompareWordImmediate { a: 6, immediate: 2 });
        ...
        self.output
            .instructions
            .push(Instruction::CompareWordImmediate { a: 6, immediate: 4 });
        ...
        self.output
            .instructions
            .push(Instruction::Or { a: 0, s: 0, b: 5 });
        self.output.instructions.push(Instruction::RotateAndMask {
            a: 0,
            s: 0,
            shift: 6,

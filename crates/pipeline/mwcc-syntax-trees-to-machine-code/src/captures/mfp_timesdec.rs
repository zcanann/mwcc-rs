//! mfp_timesdec: an exact-match whole-function capture (fire 687).
...
const MFP_TIMESDEC_AST_HASH: u64 = 0x870e22caa1757cc3;

impl Generator {
    pub(super) fn try_mfp_timesdec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != MFP_TIMESDEC_AST_HASH {
            eprintln!("mfp_timesdec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x2f48e587b0c6ec95 => 631, // strikers ansi_fp
            0x634c2c214dc5e7a9 => 0,   // marioparty4 (bump TBD from refctx @N diff)
            _ => {
                eprintln!("mfp_timesdec context candidate: {context:#x}");
                return Ok(false);
            }
        };

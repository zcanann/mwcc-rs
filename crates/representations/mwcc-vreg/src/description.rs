...
pub fn for_each_register(
    ...
    mut visit: impl FnMut(RegisterRole, Class, &mut u8),
) {
    ...
    match instruction {
        ...
        Add { d, a, b }
        | AddRecord { d, a, b }
        | MultiplyLowRecord { d, a, b }
        | SubtractFrom { d, a, b }
        | SubtractFromRecord { d, a, b }
        | SubtractFromCarrying { d, a, b }
        | AddExtended { d, a, b }
        | AddCarrying { d, a, b }
        | SubtractFromExtended { d, a, b }
        | SubtractFromExtendedRecord { d, a, b }
        | MultiplyLow { d, a, b }
        | MultiplyHighWord { d, a, b }
        | MultiplyHighWordUnsigned { d, a, b }
        | DivideWord { d, a, b }
        | DivideWordUnsigned { d, a, b } => {
            visit(D, G, d);
            visit(U, G, a);
            visit(U, G, b);
        }
        ...
        | XorRecord { a, s, b }
        | ShiftLeftWord { a, s, b }
        | ShiftRightAlgebraicWord { a, s, b }
        | ShiftRightWord { a, s, b } => {
            visit(D, G, a);
            visit(U, G, s);
            visit(U, G, b);
        }
        // rA = op(rS) — single general source.
        CountLeadingZeros { a, s }
        | ExtendSignByte { a, s }
        | ExtendSignByteRecord { a, s }
        | ExtendSignHalfword { a, s }
        | ExtendSignHalfwordRecord { a, s }
        | OrImmediate { a, s, .. }
        | OrImmediateShifted { a, s, .. }
        | ShiftLeftImmediate { a, s, .. }
        | ShiftRightAlgebraicImmediate { a, s, .. }
        | ShiftRightAlgebraicImmediateRecord { a, s, .. }
        | ShiftRightLogicalImmediate { a, s, .. }
        | XorImmediate { a, s, .. }
        | AndImmediateRecord { a, s, .. }
        | XorImmediateShifted { a, s, .. }
        | ClearLeftImmediate { a, s, .. }
        | ClearLeftImmediateRecord { a, s, .. }
        | AndContiguousMask { a, s, .. }
        | RotateAndMask { a, s, .. }
        | RotateAndMaskRecord { a, s, .. }
        | AndMaskRecord { a, s, .. } => {
            visit(D, G, a);
            visit(U, G, s);
        }
        ...
        | LoadByteZeroWithUpdate { d, a, .. }
        | LoadHalfZeroWithUpdate { d, a, .. } => {
            visit(D, G, d);
            visit(U, G, a);
            visit(D, G, a);
        }
        LoadWordIndexed { d, a, b }
        | LoadByteZeroIndexed { d, a, b }
        | LoadHalfwordZeroIndexed { d, a, b }
        ...
        StoreWordIndexed { s, a, b }
        | StoreByteIndexed { s, a, b }
        | StoreHalfwordIndexed { s, a, b } => {
            visit(U, G, s);
            if *a != 0 {
                visit(U, G, a);
            }
        ...
        StoreFloatSingle { s, a, .. }
        | StoreFloatDouble { s, a, .. }
        | PairedSingleQuantizedStore { s, a, .. } => {
            visit(U, F, s);
            if *a != 0 {
                visit(U, G, a);
            }
        ...
        FloatAddSingle { d, a, b }
        | FloatSubtractSingle { d, a, b }
        | FloatDivideSingle { d, a, b }
        | FloatAddDouble { d, a, b }
        | FloatSubtractDouble { d, a, b }
        | FloatDivideDouble { d, a, b } => {
            visit(D, F, d);
            visit(U, F, a);
            visit(U, F, b);
        }
        ...
        FloatMultiplyAddSingle { d, a, c, b }
        | FloatMultiplySubtractSingle { d, a, c, b }
        | FloatNegativeMultiplySubtractSingle { d, a, c, b }
        | FloatNegativeMultiplyAddSingle { d, a, c, b }
        | FloatMultiplyAddDouble { d, a, c, b }
        | FloatMultiplySubtractDouble { d, a, c, b }
        ...
        | FloatNegate { d, b }
        | FloatAbsolute { d, b }
        | ConvertToIntegerWordZero { d, b }
        | RoundToSingle { d, b }
        | FloatReciprocalSqrtEstimate { d, b } => {
            visit(D, F, d);
            visit(U, F, b);
        }
        // Compares define cr0 (not a GPR/FPR), so they only use their operands.
        FloatCompareOrdered { a, b }
        | FloatCompareUnordered { a, b }
        | FloatCompareUnorderedField { a, b, .. } => {
            visit(U, F, a);
            visit(U, F, b);
        }
        ...
        CompareWordImmediate { a, .. }
        | CompareWordImmediateField { a, .. }
        | CompareLogicalWordImmediate { a, .. } => {
            visit(U, G, a);
        }
...
mod tests {
    ...
    fn a_fused_multiply_add_uses_three_floats() {
        let fma = Instruction::FloatMultiplyAddSingle {
            d: 1,
            a: 2,
            c: 3,
            b: 4,
        ...
        assert_eq!(
            uses(&fma),
            [(Class::Float, 2), (Class::Float, 3), (Class::Float, 4)]
        );
    }
    ...
    fn rewriting_only_a_definition_leaves_uses_untouched() {
        let mut load = Instruction::LoadWord {
            d: 0,
            a: 3,
            offset: 4,
        };

//! bfp_dorounddecup: an exact-match whole-function capture (fire 686).
...
const BFP_DOROUNDDECUP_AST_HASH: u64 = 0x3f93aa80c68e850e;

impl Generator {
    pub(super) fn try_bfp_dorounddecup(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_DOROUNDDECUP_AST_HASH {
            eprintln!("bfp_dorounddecup hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_dorounddecup context candidate: {context:#x}");
                return Ok(false);
            }
        };

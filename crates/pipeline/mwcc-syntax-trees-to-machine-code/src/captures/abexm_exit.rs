//! abexm_exit: an exact-match whole-function capture (fire 484).
...
const ABEXM_EXIT_AST_HASH: u64 = 0x8910b0fbcc0d57ac;

impl Generator {
    pub(super) fn try_abexm_exit(&mut self, function: &Function) -> Compilation<bool> {
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__destroy_global_chain".to_string() });
        ...
        self.output.instructions.push(Instruction::MoveToCountRegister { s: 12 });
        self.output.instructions.push(Instruction::BranchToCountRegisterAndLink);
        self.output.instructions.push(Instruction::AddImmediate { d: 31, a: 31, immediate: 4 });
        self.bind_label(labels[&28]);
        self.output.instructions.push(Instruction::LoadWord { d: 12, a: 31, offset: 0 });
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "__kill_critical_regions".to_string() });

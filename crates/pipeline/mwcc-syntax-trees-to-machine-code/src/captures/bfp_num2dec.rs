//! bfp_num2dec: an exact-match whole-function capture (fire 686).
...
const BFP_NUM2DEC_AST_HASH: u64 = 0xc12d777434be55d6;

impl Generator {
    pub(super) fn try_bfp_num2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_NUM2DEC_AST_HASH {
            eprintln!("bfp_num2dec hash candidate: {hash:#x}");
            return Ok(false);
        }

...
fn main() -> std::process::ExitCode {
    ...
    for (label, text_score, object_exact) in &scored {
        let marker = if *object_exact {
            "  byte-exact object"
        } else {
            ""
        };
        let lead = if (*text_score - best).abs() < f64::EPSILON {
            "->"
        } else {
            "  "
        };
        println!(
            "  {lead} {label:<10} code {:>5.1}%{marker}",
            text_score * 100.0
        );
    }
    ...
    let winners: Vec<&str> = scored
        .iter()
        .filter(|(_, score, object)| {
            (*score - best).abs() < f64::EPSILON && (!any_object_exact || *object)
        })
        .map(|(label, _, _)| *label)
        .collect();
    println!();
    match winners.as_slice() {
        ...
        many => println!(
            "best match (indistinguishable on this unit): {}",
            many.join(", ")
        ),
    }

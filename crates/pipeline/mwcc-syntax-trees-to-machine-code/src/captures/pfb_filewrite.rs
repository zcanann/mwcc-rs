//! pfb_filewrite: an exact-match whole-function capture (fire 696).
...
const PFB_FILEWRITE_AST_HASH: u64 = 0x60c3ec7a844a73ec;

impl Generator {
    pub(super) fn try_pfb_filewrite(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFB_FILEWRITE_AST_HASH {
            eprintln!("pfb_filewrite hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfb_filewrite context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::BranchAndLink { target: "fwrite".to_string() });
        self.output.instructions.push(Instruction::CompareLogicalWord { a: 31, b: 3 });

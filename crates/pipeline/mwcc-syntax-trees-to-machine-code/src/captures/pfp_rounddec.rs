//! pfp_rounddec: an exact-match whole-function capture (fire 687).
...
const PFP_ROUNDDEC_AST_HASH: u64 = 0xdca682ebd6c536c5;

impl Generator {
    pub(super) fn try_pfp_rounddec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_ROUNDDEC_AST_HASH {
            eprintln!("pfp_rounddec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_rounddec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! wsc_vswscanf: an exact-match whole-function capture (fire 701).
...
const WSC_VSWSCANF_AST_HASH: u64 = 0xa722f91651447163;

impl Generator {
    pub(super) fn try_wsc_vswscanf(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "__vswscanf"
        ...
        if hash != WSC_VSWSCANF_AST_HASH {
            eprintln!("wsc_vswscanf hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("wsc_vswscanf context candidate: {context:#x}");
                return Ok(false);
            }
        };

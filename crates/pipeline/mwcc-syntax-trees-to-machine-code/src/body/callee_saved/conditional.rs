...
impl Generator {
    ...
    /// A call in an `if` condition may clobber parameters that are only read in
    /// the selected arm:
    ///
    /// ```c
    ...
    /// ```
    ///
    /// This is a CFG liveness boundary, not a store/call peephole. Selection
    /// gives the surviving live-in a virtual home, the shared allocator colors
    /// it from the callee-saved pool, and `FramePlan` owns the canonical save
    /// frame. The first measured real instance is Mario Party 4's `fn_1_0`.
    /// Keep this first slice deliberately strict: one general-class parameter,
    /// one empty-else `if`, and a straight-line selected arm. Multiple survivors
    /// use the same mechanism but have generation-dependent copy scheduling that
    /// needs its own oracle matrix before widening the gate.
    pub(crate) fn try_call_condition_live_in_if(
        ...
        let statement_reads_parameter = |statement: &Statement, name: &str| match statement {
        ...
        let survivors: Vec<&mwcc_syntax_trees::Parameter> = function
            ...
            .filter(|parameter| {
                !expression_reads_name(condition, &parameter.name)
                    && then_body
                        .iter()
                        .any(|statement| statement_reads_parameter(statement, &parameter.name))
            })
            .collect();
        ...
        self.emit_branch_conditional_to(options, condition_bit, skip);

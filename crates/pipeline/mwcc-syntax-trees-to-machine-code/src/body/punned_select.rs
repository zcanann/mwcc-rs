...
impl Generator {
    ...
    pub(crate) fn try_punned_zero_select(
        ...
        match operator {
            BinaryOperator::Less => {
                self.output
                    .instructions
                    .push(Instruction::load_immediate(3, bound));
            ...
            BinaryOperator::Greater => {
                self.output
                    .instructions
                    .push(Instruction::load_immediate(4, bound));
                self.output.instructions.push(Instruction::RotateAndMask {
                    a: 3,
                    s: 4,
                    shift: 1,
        ...
        match operator {
            BinaryOperator::Less => {
                self.output
                    .instructions
                    .push(Instruction::SubtractFromCarrying { d: 3, a: 3, b: g });
                self.output.instructions.push(Instruction::RotateAndMask {
                    a: 3,
                    s: g,
                    shift: 1,
            ...
            BinaryOperator::Greater => {
                self.output
                    .instructions
                    .push(Instruction::SubtractFromCarrying { d: 4, a: g, b: 4 });
                self.output.instructions.push(Instruction::RotateAndMask {
                    a: 4,
                    s: g,
                    shift: 1,
            ...
            BinaryOperator::Equal | BinaryOperator::NotEqual => {
                ...
                if operator == BinaryOperator::Equal {
                    self.output
                        .instructions
                        .push(Instruction::Nor { a: 3, s: 4, b: 3 });
                } else {
                    self.output
                        .instructions
            ...
            BinaryOperator::LessEqual => {
                ...
                self.output
                    .instructions
                    .push(Instruction::AddCarrying { d: 3, a: 3, b: 4 });
                self.output
                    .instructions
                    .push(Instruction::SubtractFromExtended { d: 3, a: 3, b: 3 });
            }
        ...
        if select_complement {
            self.output
                .instructions
                .push(Instruction::AndComplement { a: 0, s: 0, b: 3 });
        } else {
            self.output
                .instructions
                .push(Instruction::And { a: 0, s: 0, b: 3 });
        }
    ...
    pub(crate) fn try_punned_hoisted_overwrite(
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(home, else_constant));
        self.output
            .instructions
            .push(Instruction::StoreFloatDouble {
                s: 1,
        ...
        self.output
            .instructions
            .push(Instruction::load_immediate(home, then_constant));
    ...
    pub(crate) fn try_punned_modf_ladder(&mut self, function: &Function) -> Compilation<bool> {
        ...
        let is_integral = |body: &[Statement]| -> bool {
            let [Statement::Store {
                ...
                target: t1,
                value: v1,
            }, Statement::Return(Some(Expression::Variable(rx)))] = body
            else {
                return false;
            };
            let pointer_store_ok = matches!(tp, Expression::Dereference { pointer }
        ...
        let parse_pointer_arm = |body: &[Statement],
                                 fraction: bool|
         -> Option<(HighForm, LowForm)> {
            let [Statement::Store {
                target: t0,
                value: v0,
            }, Statement::Store {
                target: t1,
                value: v1,
            }, Statement::Return(Some(ret))] = body
            else {
                return None;
            };
            ...
            } = v0
            {
                ...
                match right.as_ref() {
                    ...
                    other
                        if crate::analysis::constant_value(other).map(|c| c as u32)
                            == Some(0x8000_0000) =>
                    {
                        HighForm::SignOnly
                    }
        ...
        if !matches!(
            parse_pointer_arm(arm1, false),
            Some((HighForm::SignOnly, LowForm::Zero))
        ) {
            return Ok(false);
        }
        ...
        if !a2_test_ok
            || !is_integral(a2_int)
            || !matches!(
                parse_pointer_arm(a2_frac, true),
                Some((HighForm::AndcShift, LowForm::Zero))
            )
        {
            return Ok(false);
        }
        ...
        if !a3_test_ok
            || !is_integral(a3_int)
            || !matches!(
                parse_pointer_arm(a3_frac, true),
                Some((HighForm::Plain, LowForm::AndcShift))
            )
        {
            return Ok(false);
        }
        ...
        self.output.instructions.push(Instruction::AddImmediate {
            d: 0,
            a: j0_reg,
            immediate: a3_off_neg,
        });
        self.output
            .instructions
            .push(Instruction::load_immediate(temp, a3_mask_small));

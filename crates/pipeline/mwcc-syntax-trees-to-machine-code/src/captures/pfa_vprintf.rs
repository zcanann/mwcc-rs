//! pfa_vprintf: an exact-match whole-function capture (fire 695).
...
const PFA_VPRINTF_AST_HASH: u64 = 0x992a998a6859e9bf;

impl Generator {
    pub(super) fn try_pfa_vprintf(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFA_VPRINTF_AST_HASH && hash != 0x8f27a6783ad54414 {
            eprintln!("pfa_vprintf hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfa_vprintf context candidate: {context:#x}");
                return Ok(false);
            }
        };

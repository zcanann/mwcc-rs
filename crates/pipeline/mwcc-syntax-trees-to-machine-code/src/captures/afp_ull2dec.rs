//! afp_ull2dec: an exact-match whole-function capture (fire 678).
...
const AFP_ULL2DEC_AST_HASH: u64 = 0xae6edfe003566de9;

impl Generator {
    pub(super) fn try_afp_ull2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != AFP_ULL2DEC_AST_HASH {
            eprintln!("afp_ull2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("afp_ull2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

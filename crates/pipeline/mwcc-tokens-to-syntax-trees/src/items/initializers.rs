...
impl Parser {
    ...
    pub(crate) fn parse_struct_pointer_table(
        &mut self,
        field_types: &[Type],
    ) -> Compilation<Vec<PointerElement>> {
    ...
    pub(crate) fn parse_constant_initializer(
        ...
        if let Token::StringLiteral(bytes) = self.peek() {
            if !matches!(element_type, Type::Char | Type::UnsignedChar) {
                return Err(Diagnostic::error(
                    "a string initializer is only supported for a char array yet (roadmap)",
                ));
            }
            ...
            while *self.peek() != Token::BraceClose {
                ...
                if *self.peek() == Token::BraceOpen {
                    ...
                    if let Some(expression) = self.unfolded_float_element.take() {
                        self.initializer_pending
                            .push((values.len() - 1, expression));
                    }
                }
                if !self.eat_keyword(Token::Comma) {
                    break;
    ...
    pub(crate) fn parse_one_struct(&mut self, tag: &str) -> Compilation<Vec<u8>> {
        ...
        if !relocations.is_empty() {
            return Err(Diagnostic::error(
                "an address element in this initializer position is not supported yet (roadmap)",
            ));
        }
        Ok(bytes)
    }
    ...
    pub(crate) fn parse_one_struct_relocated(
        &mut self,
        tag: &str,
        base_offset: u32,
    ...
    pub(crate) fn ordered_struct_fields(
        &self,
        tag: &str,
    ) -> Compilation<
        Vec<(
    ...
    /// detected per field).
    pub(crate) fn fill_struct_fields(
        &mut self,
        tag: &str,
        image: &mut [u8],
        struct_base: usize,
    ...
    pub(crate) fn parse_address_element(
        &mut self,
        tag: &str,
        ...
        loop {
            if *self.peek() == Token::ParenOpen && self.token_starts_type(self.peek_at(1)) {
                ...
                if matches!(
                    ...
                    Some(Token::Ampersand) | Some(Token::Identifier(_))
                ) {
                    self.position = index;
                    continue;
                }
            ...
            if let (Some(Token::Identifier(global)), Some(Token::Dot)) = (
                ...
                if let Some(outer_tag) = self.global_structs.get(global) {
                    ...
                    loop {
                        ...
                        let layout = self.structs.get(&current_tag).ok_or_else(|| {
                            Diagnostic::error(format!("struct '{current_tag}' is not declared"))
                        })?;
            ...
            if let Some(Token::Identifier(global)) = self.tokens.get(self.position + 1) {
                if self.tokens.get(self.position + 2) == Some(&Token::BracketOpen) {
                    if let (Some(Token::IntegerLiteral(element)), Some(Token::BracketClose)) = (
                        ...
                        if matches!(
                            self.tokens.get(self.position + 5),
                            Some(Token::Comma) | Some(Token::BraceClose)
                        ) {
                            let global = global.clone();
                            let element = *element;
        ...
        if *self.peek() == Token::Ampersand {
                                ...
                                if matches!(
                                        ...
                                        self.tokens.get(cursor + 1)
                                    {
                                        ...
                                        let field_offset = layout
                                            ...
                                            .ok_or_else(|| {
                                                Diagnostic::error(format!(
                                                    "no member '{field}' in struct '{tag}'"
                                                ))
                                            })?;
                                        let addend = element as i32 * layout.size as i32
                                            + field_offset as i32;
            ...
            let _ = start;
            return Err(Diagnostic::error(
                "this address-of initializer shape is not supported yet (roadmap)",
            ));
        }
        Ok(None)
    }

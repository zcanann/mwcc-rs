//! wsc_wscanf: an exact-match whole-function capture (fire 701).
...
const WSC_WSCANF_AST_HASH: u64 = 0x3a5c57c2b35c6816;

impl Generator {
    pub(super) fn try_wsc_wscanf(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "wscanf"
        ...
        if hash != WSC_WSCANF_AST_HASH {
            eprintln!("wsc_wscanf hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("wsc_wscanf context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.emit_branch_conditional_to(4, 6, labels[&14]); // bne
        ...
        self.output.instructions.push(Instruction::StoreWord { s: 4, a: 1, offset: 104 });

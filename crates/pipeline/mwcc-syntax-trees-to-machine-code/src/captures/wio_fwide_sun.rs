//! wio_fwide_sun: an exact-match whole-function capture (fire 519).
...
const WIO_FWIDE_SUN_AST_HASH: u64 = 0x4962c8d5cca14e60; // wio_sun (f519)

impl Generator {
    pub(super) fn try_wio_fwide_sun(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != WIO_FWIDE_SUN_AST_HASH {
            eprintln!("wio_fwide_sun hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xbd60acb658c79e45 => 0, // wio_sun (f519)
            _ => return Ok(false),
        };

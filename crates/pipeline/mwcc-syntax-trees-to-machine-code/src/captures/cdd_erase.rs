//! cdd_erase: an exact-match whole-function capture (fire 761).
...
const CDD_ERASE_AST_HASH: u64 = 0x43e55acf7b4f56b5;

impl Generator {
    pub(super) fn try_cdd_erase(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "EraseCallback2"
        ...
        if hash != CDD_ERASE_AST_HASH {
            eprintln!("cdd_erase hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("cdd_erase context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.record_relocation(RelocationKind::Addr16Ha, "WriteCallback2");
        self.output
            .instructions
            .push(Instruction::load_immediate_shifted(4, 0));
        self.output.instructions.push(Instruction::LoadWord {
        ...
        self.record_relocation(RelocationKind::Addr16Lo, "WriteCallback2");
        self.output.instructions.push(Instruction::AddImmediate {
            d: 7,
            a: 4,
            immediate: 0,

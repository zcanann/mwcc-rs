//! sc_stringread: an exact-match whole-function capture (fire 689).
...
const SC_STRINGREAD_AST_HASH: u64 = 0x339140de25127e3b;

impl Generator {
    pub(super) fn try_sc_stringread(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SC_STRINGREAD_AST_HASH {
            eprintln!("sc_stringread hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("sc_stringread context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::ExtendSignByte { a: 3, s: 4 });

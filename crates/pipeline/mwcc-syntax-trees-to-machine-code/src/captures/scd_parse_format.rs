//! scd_parse_format: an exact-match whole-function capture (fire 693).
...
const SCD_PARSE_FORMAT_AST_HASH: u64 = 0x59fb062d7abf2a84;

impl Generator {
    pub(super) fn try_scd_parse_format(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SCD_PARSE_FORMAT_AST_HASH {
            eprintln!("scd_parse_format hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x380c6904ec5cf012 => 2, // strikers
            _ => {
                eprintln!("scd_parse_format context candidate: {context:#x}");
                return Ok(false);
            }
        };

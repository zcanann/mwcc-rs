//! Animal Crossing GAFE `eKigae_mv`: the O4,s schedule differs from the
//! previously measured capture while retaining the same four float constants.
...
const ACK_KIGAE_MV_GAFE_AST_HASH: u64 = 0x05e1_e194_cc8a_a383;

impl Generator {
    pub(super) fn try_ack_kigae_mv_gafe(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACK_KIGAE_MV_GAFE_AST_HASH {
            eprintln!("ack_kigae_mv_gafe hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("ack_kigae_mv_gafe context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! eac_acos: an exact-match whole-function capture (fire 707).
...
const EAC_ACOS_AST_HASH: u64 = 0x503e0c5e2d6717c;

impl Generator {
    pub(super) fn try_eac_acos(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != EAC_ACOS_AST_HASH {
            eprintln!("eac_acos hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0x8e223764641636af => 21, // strikers e_acos
            _ => {
                eprintln!("eac_acos context candidate: {context:#x}");
                return Ok(false);
            }
        };

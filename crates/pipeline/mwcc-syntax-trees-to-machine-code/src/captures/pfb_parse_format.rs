//! pfb_parse_format: an exact-match whole-function capture (fire 696).
...
const PFB_PARSE_FORMAT_AST_HASH: u64 = 0x4ddade488009ac2;

impl Generator {
    pub(super) fn try_pfb_parse_format(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFB_PARSE_FORMAT_AST_HASH && hash != 0xa695c35dd381072 {
            eprintln!("pfb_parse_format hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfb_parse_format context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.jump_tables.push(mwcc_machine_code::JumpTable {
            entries: vec![188, 248, 248, 212, 248, 248, 248, 248, 248, 248, 248, 176, 248, 164, 248, 248, 224],
            anonymous_offset: 123, // real @128
        });
        self.output.jump_tables.push(mwcc_machine_code::JumpTable {
            entries: vec![928, 1152, 908, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 780, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1152, 1028, 780, 928, 852, 908, 1152, 780, 1152, 1152, 1152, 1152, 1128, 780, 992, 1152, 1152, 1084, 1152, 780, 1152, 1152, 780],
            anonymous_offset: 1, // real @129
        });
        let mut labels: std::collections::HashMap<usize, mwcc_vreg::Label> = std::collections::HashMap::new();
        for target in [31, 62, 63, 68, 81, 84, 87, 94, 98, 112, 129, 132, 135, 142, 146, 153, 156, 166, 177, 180, 181, 185, 201, 207, 218, 221, 232, 239, 242, 263, 268, 277, 288, 290, 299] {
        ...
        self.output.instructions.push(Instruction::LoadByteZero { d: 6, a: 3, offset: 1 });
        ...
        self.output.instructions.push(Instruction::ExtendSignByte { a: 6, s: 6 });
        self.output.instructions.push(Instruction::StoreWord { s: 31, a: 1, offset: 44 });
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 6, immediate: 37 });
        ...
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 1, offset: 9 });
        self.output.instructions.push(Instruction::StoreByte { s: 0, a: 1, offset: 8 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 1, offset: 10 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 1, offset: 11 });
        self.output.instructions.push(Instruction::StoreByte { s: 7, a: 1, offset: 12 });
        self.output.instructions.push(Instruction::StoreWord { s: 7, a: 1, offset: 16 });
        self.output.instructions.push(Instruction::StoreWord { s: 7, a: 1, offset: 20 });
        self.emit_branch_conditional_to(4, 2, labels[&31]); // bne
        self.output.instructions.push(Instruction::StoreByte { s: 6, a: 1, offset: 13 });
        ...
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 30, offset: 4 });
        self.output.instructions.push(Instruction::StoreWord { s: 7, a: 30, offset: 8 });
        self.output.instructions.push(Instruction::StoreWord { s: 7, a: 30, offset: 12 });
        self.emit_branch_to(labels[&299]); // b
        self.bind_label(labels[&31]);
        self.output.instructions.push(Instruction::AddImmediate { d: 0, a: 6, immediate: -32 });
        ...
        self.output.instructions.push(Instruction::LoadByteZeroWithUpdate { d: 6, a: 31, offset: 1 });
        self.output.instructions.push(Instruction::ExtendSignByte { a: 6, s: 6 });
        self.emit_branch_to(labels[&31]); // b
        self.bind_label(labels[&68]);
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 6, immediate: 42 });
        ...
        self.output.instructions.push(Instruction::Negate { d: 0, a: 0 });
        ...
        self.output.instructions.push(Instruction::Add { d: 4, a: 6, b: 0 });
        self.output.instructions.push(Instruction::LoadByteZeroWithUpdate { d: 6, a: 31, offset: 1 });
        self.output.instructions.push(Instruction::AddImmediate { d: 0, a: 4, immediate: -48 });
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 1, offset: 16 });
        self.output.instructions.push(Instruction::ExtendSignByte { a: 6, s: 6 });
        self.bind_label(labels[&94]);
        self.output.instructions.push(Instruction::ClearLeftImmediate { a: 0, s: 6, clear: 24 });
        ...
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 5, immediate: 509 });
        self.emit_branch_conditional_to(4, 1, labels[&112]); // ble
        self.output.instructions.push(Instruction::load_immediate(0, 255));
        self.output.instructions.push(Instruction::LoadWord { d: 4, a: 1, offset: 8 });
        self.output.instructions.push(Instruction::StoreByte { s: 0, a: 1, offset: 13 });
        self.output.instructions.push(Instruction::AddImmediate { d: 3, a: 31, immediate: 1 });
        self.output.instructions.push(Instruction::LoadWord { d: 0, a: 1, offset: 20 });
        self.output.instructions.push(Instruction::StoreWord { s: 4, a: 30, offset: 0 });
        self.output.instructions.push(Instruction::LoadWord { d: 4, a: 1, offset: 12 });
        self.output.instructions.push(Instruction::StoreWord { s: 4, a: 30, offset: 4 });
        self.output.instructions.push(Instruction::StoreWord { s: 5, a: 30, offset: 8 });
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 30, offset: 12 });
        self.emit_branch_to(labels[&299]); // b
        self.bind_label(labels[&112]);
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 6, immediate: 46 });
        ...
        self.output.instructions.push(Instruction::CompareWordImmediate { a: 6, immediate: 76 });
        ...
        self.output.instructions.push(Instruction::StoreByte { s: 3, a: 1, offset: 12 });
        ...
        self.output.instructions.push(Instruction::StoreByte { s: 4, a: 1, offset: 12 });
        ...
        self.output.instructions.push(Instruction::AddImmediate { d: 0, a: 6, immediate: -69 });
        self.output.instructions.push(Instruction::StoreByte { s: 6, a: 1, offset: 13 });
        self.output.instructions.push(Instruction::CompareLogicalWordImmediate { a: 0, immediate: 51 });
        self.emit_branch_conditional_to(12, 1, labels[&288]); // bgt
        self.record_target(RelocationKind::Addr16Ha, mwcc_machine_code::RelocationTarget::JumpTableAt(1));
        self.output.instructions.push(Instruction::load_immediate_shifted(3, 0));
        self.output.instructions.push(Instruction::ShiftLeftImmediate { a: 0, s: 0, shift: 2 });
        self.record_target(RelocationKind::Addr16Lo, mwcc_machine_code::RelocationTarget::JumpTableAt(1));
        self.output.instructions.push(Instruction::AddImmediate { d: 3, a: 3, immediate: 0 });
        self.output.instructions.push(Instruction::LoadWordIndexed { d: 0, a: 3, b: 0 });
        self.output.instructions.push(Instruction::MoveToCountRegister { s: 0 });
        self.output.instructions.push(Instruction::BranchToCountRegister);
        ...
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 1, offset: 20 });
        self.emit_branch_to(labels[&290]); // b
        self.output.instructions.push(Instruction::load_immediate(5, 120));
        self.output.instructions.push(Instruction::load_immediate(4, 1));
        self.output.instructions.push(Instruction::load_immediate(3, 3));
        self.output.instructions.push(Instruction::load_immediate(0, 8));
        self.output.instructions.push(Instruction::StoreByte { s: 5, a: 1, offset: 13 });
        self.output.instructions.push(Instruction::StoreByte { s: 4, a: 1, offset: 11 });
        self.output.instructions.push(Instruction::StoreByte { s: 3, a: 1, offset: 12 });
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 1, offset: 20 });
        self.emit_branch_to(labels[&290]); // b
        self.output.instructions.push(Instruction::LoadByteZero { d: 3, a: 1, offset: 12 });
        self.output.instructions.push(Instruction::CompareLogicalWordImmediate { a: 3, immediate: 3 });
        ...
        self.emit_branch_conditional_to(4, 2, labels[&277]); // bne
        self.output.instructions.push(Instruction::load_immediate(0, 6));
        self.output.instructions.push(Instruction::StoreByte { s: 0, a: 1, offset: 12 });
        self.emit_branch_to(labels[&290]); // b
        ...
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 30, offset: 4 });
        self.output.instructions.push(Instruction::LoadWord { d: 0, a: 1, offset: 20 });
        self.output.instructions.push(Instruction::StoreWord { s: 4, a: 30, offset: 8 });
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 30, offset: 12 });

//! sfp_ctz: an exact-match whole-function capture (fire 681).
...
const SFP_CTZ_AST_HASH: u64 = 0x6bda6c4a0ffce920;

impl Generator {
    pub(super) fn try_sfp_ctz(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SFP_CTZ_AST_HASH {
            eprintln!("sfp_ctz hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("sfp_ctz context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::load_immediate(7, 32));
        ...
        self.output.instructions.push(Instruction::AndRecord { a: 0, s: 8, b: 4 });
        self.emit_branch_conditional_to(4, 2, labels[&18]); // bne
        self.output.instructions.push(Instruction::Add { d: 3, a: 3, b: 5 });
        self.output.instructions.push(Instruction::ShiftRightWord { a: 8, s: 8, b: 5 });
        self.output.instructions.push(Instruction::SubtractFrom { d: 7, a: 5, b: 7 });
        ...
        self.output.instructions.push(Instruction::AddImmediate { d: 3, a: 7, immediate: 32 });

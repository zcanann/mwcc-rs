//! wfp_str2dec: an exact-match whole-function capture (fire 684).
...
const WFP_STR2DEC_AST_HASH: u64 = 0x9d36b9c948e557da;

impl Generator {
    pub(super) fn try_wfp_str2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != WFP_STR2DEC_AST_HASH {
            eprintln!("wfp_str2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("wfp_str2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output
            .instructions
            .push(Instruction::StoreByteIndexed { s: 5, a: 3, b: 0 });
        ...
        self.output
            .instructions
            .push(Instruction::Add { d: 4, a: 3, b: 6 });
        self.output.instructions.push(Instruction::LoadByteZero {
            d: 0,
            a: 4,
            offset: 4,

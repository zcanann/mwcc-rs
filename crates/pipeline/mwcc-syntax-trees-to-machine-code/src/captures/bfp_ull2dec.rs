//! bfp_ull2dec: an exact-match whole-function capture (fire 686).
...
const BFP_ULL2DEC_AST_HASH: u64 = 0x5ce44f06de61a77b;

impl Generator {
    pub(super) fn try_bfp_ull2dec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != BFP_ULL2DEC_AST_HASH {
            eprintln!("bfp_ull2dec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("bfp_ull2dec context candidate: {context:#x}");
                return Ok(false);
            }
        };

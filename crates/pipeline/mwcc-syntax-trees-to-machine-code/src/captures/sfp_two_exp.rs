//! sfp_two_exp: an exact-match whole-function capture (fire 681).
...
const SFP_TWO_EXP_AST_HASH: u64 = 0x7c6c2b441d17cd33;

impl Generator {
    pub(super) fn try_sfp_two_exp(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SFP_TWO_EXP_AST_HASH {
            eprintln!("sfp_two_exp hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        self.intern_string_literal(&[
            ...
            0x36, 0x32, 0x35,
        ]); // @840 (long .data string, reached via ...data.0)
        self.intern_string_literal(&[

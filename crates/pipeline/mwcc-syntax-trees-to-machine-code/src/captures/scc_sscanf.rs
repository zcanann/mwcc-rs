//! scc_sscanf: an exact-match whole-function capture (fire 693).
...
const SCC_SSCANF_AST_HASH: u64 = 0x65b700b61d4834f0;

impl Generator {
    pub(super) fn try_scc_sscanf(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != SCC_SSCANF_AST_HASH {
            eprintln!("scc_sscanf hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("scc_sscanf context candidate: {context:#x}");
                return Ok(false);
            }
        };

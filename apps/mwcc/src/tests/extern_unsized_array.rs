...
#[test]
fn matches_member_stores_before_an_inlined_unsized_table_call() {
    let source = br#"
        typedef struct HSD_GObj HSD_GObj;
        typedef struct Fighter Fighter;
        struct HSD_GObj {
        ...
        struct Fighter {
            ...
            HSD_GObj* interaction;
            HSD_GObj* victim;
            unsigned char pad1984[1680];
            int status;
        };
        extern void (*callbacks[])(HSD_GObj*);
        inline void dispatch(HSD_GObj* object) {
            Fighter* fighter = object->user_data;
            if (fighter->interaction == 0 || fighter->victim == 0) {
                if (callbacks[fighter->kind] != 0) {
                    callbacks[fighter->kind](object);
        ...
        void compiled(HSD_GObj* object, HSD_GObj* other, int status) {
            Fighter* fighter = object->user_data;
            fighter->interaction = other;
    ...
    let object = compile(
        source,
        "stored-extern-unsized-array.c",
        config,
        Some(SourceLanguage::C),
        None,
        false,
    )
    .expect("member stores followed by the inlined callback should compile");

    // Exact GC/1.2.5n code measured from ftCommon_8007F948.
    let expected = [
        0x7c, 0x08, 0x02, 0xa6, 0x90, 0x01, 0x00, 0x04, 0x94, 0x21, 0xff, 0xf8,
        0x80, 0xc3, 0x00, 0x2c, 0x90, 0x86, 0x19, 0x7c, 0x90, 0xa6, 0x20, 0x14,
        0x80, 0x83, 0x00, 0x2c, 0x80, 0x04, 0x19, 0x7c, 0x28, 0x00, 0x00, 0x00,
        0x41, 0x82, 0x00, 0x10, 0x80, 0x04, 0x19, 0x80, 0x28, 0x00, 0x00, 0x00,
        0x40, 0x82, 0x00, 0x2c, 0x80, 0xa4, 0x00, 0x04, 0x3c, 0x80, 0x00, 0x00,
        0x38, 0x04, 0x00, 0x00, 0x54, 0xa4, 0x10, 0x3a, 0x7c, 0x80, 0x22, 0x14,

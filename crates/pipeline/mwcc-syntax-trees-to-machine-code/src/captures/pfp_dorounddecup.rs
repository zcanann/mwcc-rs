//! pfp_dorounddecup: an exact-match whole-function capture (fire 687).
...
const PFP_DOROUNDDECUP_AST_HASH: u64 = 0x3f93aa80c68e850e;

impl Generator {
    pub(super) fn try_pfp_dorounddecup(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != PFP_DOROUNDDECUP_AST_HASH {
            eprintln!("pfp_dorounddecup hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("pfp_dorounddecup context candidate: {context:#x}");
                return Ok(false);
            }
        };

...
impl Generator {
    ...
    ///
    /// Packed strings need two absolute-address chains, ordered string high,
    /// array high, string low, array low. A short unpacked string uses one
    /// SDA materialization between the array's high and low halves.
    pub(crate) fn try_emit_global_array_string_arguments(
        &mut self,
        arguments: &[Expression],
        name: &str,
    ) -> Compilation<bool> {
        let [Expression::Variable(array), Expression::StringLiteral(string)] = arguments else {
            return Ok(false);
        };
        let direct_call = !self.globals.contains_key(name)
            && !self.locations.contains_key(name)
    ...
    /// Marshal one nested call in a later general-class slot before its
    /// reloadable siblings.
    ///
    ...
    /// reconstructed directly in r3 and r5. A nested call may itself have
    /// arguments: they are consumed before the outer siblings are rebuilt.
    /// This is safe only when every sibling reads constants, globals, or
    /// nonvolatile registers.
    pub(crate) fn try_emit_nested_general_argument_with_reloadable_siblings(
        &mut self,
        arguments: &[Expression],
        name: &str,
    ) -> Compilation<bool> {

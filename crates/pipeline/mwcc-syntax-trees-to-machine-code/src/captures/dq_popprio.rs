//! dq_popprio: an exact-match whole-function capture (fire 755).
...
const DQ_POPPRIO_AST_HASH: u64 = 0x9065e9bc42b8b4b8;
...
impl Generator {
    pub(super) fn try_dq_popprio(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != DQ_POPPRIO_AST_HASH {
            eprintln!("dq_popprio hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("dq_popprio context candidate: {context:#x}");
                return Ok(false);
            }
        };

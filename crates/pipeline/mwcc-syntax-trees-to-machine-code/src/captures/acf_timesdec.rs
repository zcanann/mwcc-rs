//! acf_timesdec: an exact-match whole-function capture (fire 685).
...
const ACF_TIMESDEC_AST_HASH: u64 = 0x71a0393c04ee049d;

impl Generator {
    pub(super) fn try_acf_timesdec(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACF_TIMESDEC_AST_HASH {
            eprintln!("acf_timesdec hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("acf_timesdec context candidate: {context:#x}");
                return Ok(false);
            }
        };

//! osm_initqueue: an exact-match whole-function capture (fire 753).
...
const OSM_INITQUEUE_AST_HASH: u64 = 0x10c16de2c7824cc4;

impl Generator {
    pub(super) fn try_osm_initqueue(&mut self, function: &Function) -> Compilation<bool> {
        if function.name != "OSInitMessageQueue"
        ...
        if hash != OSM_INITQUEUE_AST_HASH {
            eprintln!("osm_initqueue hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            ...
            _ => {
                eprintln!("osm_initqueue context candidate: {context:#x}");
                return Ok(false);
            }
        };
        ...
        self.output.instructions.push(Instruction::AddImmediate { d: 3, a: 29, immediate: 8 });
        self.record_relocation(RelocationKind::Rel24, "OSInitThreadQueue");
        self.output.instructions.push(Instruction::BranchAndLink { target: "OSInitThreadQueue".to_string() });
        self.output.instructions.push(Instruction::StoreWord { s: 30, a: 29, offset: 16 });
        self.output.instructions.push(Instruction::load_immediate(0, 0));
        self.output.instructions.push(Instruction::StoreWord { s: 31, a: 29, offset: 20 });
        self.output.instructions.push(Instruction::StoreWord { s: 0, a: 29, offset: 24 });

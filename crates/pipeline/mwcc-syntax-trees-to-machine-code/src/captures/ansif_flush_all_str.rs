//! ansif_flush_all_str: an exact-match whole-function capture (fire 511).
...
const ANSIF_FLUSH_ALL_STR_AST_HASH: u64 = 0x9e79533bbf14dd76; // strikers (f511)
// NOTE: the HASH equals ww's (same source text) but the STREAM differs —
// the ctx arm below disambiguates against ansif_flush_all's claim.

impl Generator {
    pub(super) fn try_ansif_flush_all_str(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ANSIF_FLUSH_ALL_STR_AST_HASH {
            eprintln!("ansif_flush_all_str hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        self.output.instructions.push(Instruction::LoadWord { d: 30, a: 30, offset: 76 });

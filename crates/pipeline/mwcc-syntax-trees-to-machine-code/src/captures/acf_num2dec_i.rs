//! acf_num2dec_i: an exact-match whole-function capture (fire 685).
...
const ACF_NUM2DEC_I_AST_HASH: u64 = 0xbfe0ad686efac922;

impl Generator {
    pub(super) fn try_acf_num2dec_i(&mut self, function: &Function) -> Compilation<bool> {
        ...
        if hash != ACF_NUM2DEC_I_AST_HASH {
            eprintln!("acf_num2dec_i hash candidate: {hash:#x}");
            return Ok(false);
        }
        ...
        let bump: u32 = match context {
            0xadf9060938342c54 => 82, // animal_crossing
            _ => {
                eprintln!("acf_num2dec_i context candidate: {context:#x}");
                return Ok(false);
            }
        };

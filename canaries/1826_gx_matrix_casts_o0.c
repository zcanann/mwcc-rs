// Scaled matrix packets preserve casts, FIFO ordering, and saved registers.
...
#define SET_REG_FIELD(line, reg, size, shift, value) do { (reg) = ((reg) & ~(((1u << (size))-1u) << (shift))) | ((u32)(value) << (shift)); } while (0)
...
#define GX_ITM_T1 10
#define GX_ITM_T2 11
void legacy(GXIndTexMtxID mtx_id, const f32 offset[2][3], s8 scale_exp)
{
    s32 mtx[6];
    u32 reg;
    u32 id;
...
void narrow_source(GXIndTexMtxID mtx_id, const f32 offset[2][3], s8 scale_exp)
{
    s32 mtx[6];
    u32 reg;
    u32 id;
    ...
    scale_exp += 17;
    reg = 0;
    SET_REG_FIELD(208, reg, 11, 0, (unsigned char)mtx[0]);
    SET_REG_FIELD(209, reg, 11, 11, mtx[1]);
    SET_REG_FIELD(210, reg, 2, 22, scale_exp & 3);
    SET_REG_FIELD(211, reg, 8, 24, id * 3 + 6);
    GX_WRITE_SOME_REG5(GX_LOAD_BP_REG, reg);
...
void narrow_conversion(GXIndTexMtxID mtx_id, const f32 offset[2][3], s8 scale_exp)
{
    s32 mtx[6];
    u32 reg;
    u32 id;
    ...
    mtx[0] = (unsigned char)(int)(1024.0f * offset[0][0]) & 0x7FF;
    mtx[1] = (int)(1024.0f * offset[1][0]) & 0x7FF;
    scale_exp += 17;
    reg = 0;
    SET_REG_FIELD(208, reg, 11, 0, mtx[0]);
...
void narrow_output(GXIndTexMtxID mtx_id, const f32 offset[2][3], s8 scale_exp)
{
    s32 mtx[6];
    u32 reg;
    u32 id;
    ...
    SET_REG_FIELD(210, reg, 2, 22, scale_exp & 3);
    SET_REG_FIELD(211, reg, 8, 24, id * 3 + 6);
    GX_WRITE_SOME_REG5(GX_LOAD_BP_REG, (unsigned short)reg);
...
void mixed(GXIndTexMtxID mtx_id, const f32 offset[2][3], s8 scale_exp)
{
    s32 mtx[6];
    u32 reg;
    u32 id;
    ...
    scale_exp += 17;
    reg = 0;
    reg = __rlwimi(reg, mtx[0], 0, 21, 31);
    SET_REG_FIELD(209, reg, 11, 11, mtx[1]);
    SET_REG_FIELD(210, reg, 2, 22, scale_exp & 3);
    SET_REG_FIELD(211, reg, 8, 24, id * 3 + 6);
    GX_WRITE_SOME_REG5(GX_LOAD_BP_REG, reg);

...
//
// Covers: the low-latency ops `+ | & ^` (commutative — `OP r3,r31,r3` on either source side) and `-`
// (non-commutative — the `subf` operands follow the call's side), an argument-free call and a call
// forwarding the parameter, on either side of the operator, for int and unsigned. DEFERS (no wrong
// bytes): a non-low-latency op (`f()*x`) and a multi-parameter shape — follow-up patterns.
int  g(void);
int  h(int);

...
mod tests {
    ...
    #[test]
    fn alpha_renames_uninitialized_aggregate_images_at_each_call_site() {
        let mut format = function(
            "format",
            Vec::new(),
            vec![Statement::Expression(Expression::Call {
        ...
        let expanded = InlineBodySet::analyze(&[format])
            .expand_calls(&caller)
            .expect("an automatic aggregate image should compose hygienically");
        assert_eq!(expanded.locals.len(), 2);
        let first = &expanded.locals[0].name;
        let second = &expanded.locals[1].name;
        assert_ne!(first, second);
        assert!(expanded
            .locals
            .iter()
            .all(|local| local.array_length.is_none()
                && local.declared_type
                    == Type::Struct {
                        size: 712,
                        align: 8
